...
boundaries for model, model IO envelopes, memory stores, turn, tool, agent, workflow, evaluation, replay, trace,
...
| FinRobot examples | `go run ./cmd/leia examples check --jobs=6 examples/ai/finrobot_translation` | Pass: `84 ok, 0 skipped, 0 failed` |
...
| `go run ./cmd/leia examples list --json` filtered to `examples/ai/finrobot_translation/` | `84` examples |
| Same example inventory grouped by runner | `73` `host-vm`, `6` `llm-replay`, `5` `evaluate` |
| Same example inventory filtered to `/live_packages/` | `38` registered live-package examples, including the standalone generic memory store package |
| Same example inventory filtered to `/live_packages/generic_` | `16` registered generic AI live-package examples |
...
The file-inventory count was refreshed to 653 `rg --files` inventory files. `AI_DIALECT_GAPS.md` is
...
generic model, model IO envelope, document RAG, prompt/role catalog, turn,
tool, agent, workflow, evaluation, replay, trace, approval, and package-audit boundaries have registered live-package examples, and the
...
`go run ./cmd/leia examples list --json` discovers 84 runnable/checkable FinRobot
...
- 5 `evaluate` examples
- 38 top-level live-package skeleton examples:
...
```text
examples: 84 ok, 0 skipped, 0 failed
```
...
`codex/ai-dialect-polish` surface: 84 registered runnable/checkable
examples, 653 files in the translation directory, and 38 checked-in
