...
| FinRobot examples | `go run ./cmd/leia examples check --jobs=6 examples/ai/finrobot_translation` | Pass: `83 ok, 0 skipped, 0 failed` |
...
| `go run ./cmd/leia examples list --json` filtered to `examples/ai/finrobot_translation/` | `83` examples |
| Same example inventory grouped by runner | `72` `host-vm`, `6` `llm-replay`, `5` `evaluate` |
| Same example inventory filtered to `/live_packages/` | `37` registered live-package examples, including the standalone generic memory store package |
| Same example inventory filtered to `/live_packages/generic_` | `15` registered generic AI live-package examples |
...
The file-inventory count was refreshed to 645 `rg --files` inventory files. `AI_DIALECT_GAPS.md` is
...
generic model, model IO envelope, document RAG, turn, tool, agent, workflow,
evaluation, replay, trace, approval, and package-audit boundaries have registered live-package examples, and the
...
`go run ./cmd/leia examples list --json` discovers 83 runnable/checkable FinRobot
...
- 5 `evaluate` examples
- 37 top-level live-package skeleton examples:
...
```text
examples: 83 ok, 0 skipped, 0 failed
```
...
`codex/ai-dialect-polish` surface: 83 registered runnable/checkable
examples, 645 files in the translation directory, and 37 checked-in
