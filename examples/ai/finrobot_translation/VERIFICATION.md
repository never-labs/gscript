...
prompt/role catalogs, evidence/report artifacts, UI snapshot evaluation, chart
...
| FinRobot examples | `go run ./cmd/leia examples check --jobs=6 examples/ai/finrobot_translation` | Pass: `91 ok, 0 skipped, 0 failed` |
...
| Same example inventory grouped by runner | `80` `host-vm`, `6` `llm-replay`, `5` `evaluate` |
| Same example inventory filtered to `/live_packages/` | `45` registered live-package examples, including the standalone generic memory store package |
| Same example inventory filtered to `/live_packages/generic_` | `23` registered generic AI live-package examples |
...
The file-inventory count was refreshed to 715 `rg --files` inventory files. `AI_DIALECT_GAPS.md` is
...
evidence/report artifact, UI snapshot evaluator, chart render contracts, optional adapter boundary, product app boundary, turn, tool, agent, workflow, evaluation, replay,
...
`go run ./cmd/leia examples list --json` discovers 91 runnable/checkable FinRobot
...
- 5 `evaluate` examples
- 45 top-level live-package skeleton examples:
...
```text
examples: 91 ok, 0 skipped, 0 failed
```
...
`codex/ai-dialect-polish` surface: 91 registered runnable/checkable
examples, 715 files in the translation directory, and 42 checked-in
