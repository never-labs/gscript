...
prompt/role catalogs, evidence/report artifacts, report render contracts, UI snapshot evaluation, chart
...
| FinRobot examples | `go run ./cmd/leia examples check --jobs=6 examples/ai/finrobot_translation` | Pass: `92 ok, 0 skipped, 0 failed` |
...
| `go run ./cmd/leia examples list --json` filtered to `examples/ai/finrobot_translation/` | `88` examples |
| Same example inventory grouped by runner | `81` `host-vm`, `6` `llm-replay`, `5` `evaluate` |
| Same example inventory filtered to `/live_packages/` | `46` registered live-package examples, including the standalone generic memory store package |
| Same example inventory filtered to `/live_packages/generic_` | `24` registered generic AI live-package examples |
...
The file-inventory count was refreshed to 725 `rg --files` inventory files. `AI_DIALECT_GAPS.md` is
...
evidence/report artifact, report render contracts, UI snapshot evaluator, chart render contracts, optional adapter boundary, product app boundary, turn, tool, agent, workflow, evaluation, replay,
...
`go run ./cmd/leia examples list --json` discovers 92 runnable/checkable FinRobot
...
- 5 `evaluate` examples
- 46 top-level live-package skeleton examples:
...
```text
examples: 92 ok, 0 skipped, 0 failed
```
...
`codex/ai-dialect-polish` surface: 92 registered runnable/checkable
examples, 725 files in the translation directory, and 42 checked-in
