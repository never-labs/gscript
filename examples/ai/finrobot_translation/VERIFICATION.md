...
render contracts, memory stores, turn, tool, agent, workflow, evaluation, replay, trace,
...
| FinRobot examples | `go run ./cmd/leia examples check --jobs=6 examples/ai/finrobot_translation` | Pass: `88 ok, 0 skipped, 0 failed` |
...
| Same example inventory grouped by runner | `77` `host-vm`, `6` `llm-replay`, `5` `evaluate` |
| Same example inventory filtered to `/live_packages/` | `42` registered live-package examples, including the standalone generic memory store package |
| Same example inventory filtered to `/live_packages/generic_` | `20` registered generic AI live-package examples |
...
The file-inventory count was refreshed to 685 `rg --files` inventory files. `AI_DIALECT_GAPS.md` is
...
evidence/report artifact, UI snapshot evaluator, chart render contracts, turn, tool, agent, workflow, evaluation, replay,
...
`go run ./cmd/leia examples list --json` discovers 88 runnable/checkable FinRobot
...
- 5 `evaluate` examples
- 42 top-level live-package skeleton examples:
...
```text
examples: 88 ok, 0 skipped, 0 failed
```
...
`codex/ai-dialect-polish` surface: 88 registered runnable/checkable
examples, 685 files in the translation directory, and 42 checked-in
