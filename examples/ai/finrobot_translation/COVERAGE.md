...
- Current translation directory file count: 785 files, including 50
  provider-free live-package skeleton directories and the upstream coverage
  ledger artifact.
- Registered examples: 96 runnable/checkable examples discovered by
  `go run ./cmd/leia examples list --json`.
- Fixture index normalization: 49 live-package skeletons use
...
examples, and tests. Current inventory finds 28 registered generic live-package
...
| Command | Result |
| --- | --- |
| `rg --files examples/ai/finrobot_translation \| wc -l \| tr -d ' '` | `785` files |
| `go run ./cmd/leia examples list --json` filtered to `examples/ai/finrobot_translation/` | `96` examples: `85` `host-vm`, `6` `llm-replay`, `5` `evaluate` |
| `find examples/ai/finrobot_translation/live_packages -mindepth 1 -maxdepth 1 -type d ...` | `50` live-package skeleton directories |
| `find examples/ai/finrobot_translation/live_packages -path '*/fixtures/provider_free_fixture_index.json' -type f ...` | `49` provider-free fixture indexes |
| `go run ./cmd/leia examples list --json` filtered to `/live_packages/generic_` | `28` registered generic AI live-package examples |
...
This refresh updated the file-inventory count to 785 `rg --files` inventory files. The registered
...
Runner summary: 85 `host-vm`, 6 `llm-replay`, 5 `evaluate`, 96 runnable,
96 checkable.
...
| --- | --- | --- |
| Registered example inventory | 96 registered examples under `examples/ai/finrobot_translation`; all runnable/checkable | Complete |
| File inventory | 785 files in the translation directory, including checked-in live-package skeleton directories, status docs, and the upstream coverage ledger | Complete |
...
| Host-VM contract examples | 85 `host-vm` examples for config, tools, schemas, API replay, reports, compliance, packaging, and live-package skeleton contracts | Complete |
...
| Live-package skeletons | 50 checked-in skeleton directories, including 28 reusable `live_packages/generic_*` AI boundaries and 22 FinRobot/finance package boundaries; all 50 include registered `.leia` examples | Complete |
...
96-example translation slice, including live-package skeleton contracts.
