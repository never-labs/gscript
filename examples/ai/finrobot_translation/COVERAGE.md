...
- Current translation directory file count: 735 files, including 47
  provider-free live-package skeleton directories and the upstream coverage
  ledger artifact.
- Registered examples: 93 runnable/checkable examples discovered by
  `go run ./cmd/leia examples list --json`.
- Fixture index normalization: 46 live-package skeletons use
...
examples, and tests. Current inventory finds 25 registered generic live-package
examples covering model resolution, model IO envelopes, coding workspaces,
data provider boundaries, document RAG, evidence/report artifacts, report render
contracts, event intelligence boundaries, UI snapshot evaluation, chart render
...
| Command | Result |
| --- | --- |
| `rg --files examples/ai/finrobot_translation \| wc -l \| tr -d ' '` | `735` files |
| `go run ./cmd/leia examples list --json` filtered to `examples/ai/finrobot_translation/` | `93` examples: `82` `host-vm`, `6` `llm-replay`, `5` `evaluate` |
| `find examples/ai/finrobot_translation/live_packages -mindepth 1 -maxdepth 1 -type d ...` | `47` live-package skeleton directories |
| `find examples/ai/finrobot_translation/live_packages -path '*/fixtures/provider_free_fixture_index.json' -type f ...` | `46` provider-free fixture indexes |
| `go run ./cmd/leia examples list --json` filtered to `/live_packages/generic_` | `25` registered generic AI live-package examples |
...
This refresh updated the file-inventory count to 735 `rg --files` inventory files. The registered
example, runner, live-package, generic live-package, and tutorial runnable
counts now include the generic event intelligence boundary package.
...
Runner summary: 82 `host-vm`, 6 `llm-replay`, 5 `evaluate`, 93 runnable,
93 checkable.
...
| --- | --- | --- |
| Registered example inventory | 93 registered examples under `examples/ai/finrobot_translation`; all runnable/checkable | Complete |
| File inventory | 735 files in the translation directory, including checked-in live-package skeleton directories, status docs, and the upstream coverage ledger | Complete |
...
| Host-VM contract examples | 82 `host-vm` examples for config, tools, schemas, API replay, reports, compliance, packaging, and live-package skeleton contracts | Complete |
...
| Live-package skeletons | 47 checked-in skeleton directories, including 25 reusable `live_packages/generic_*` AI boundaries and 22 FinRobot/finance package boundaries; all 47 include registered `.leia` examples | Complete |
...
93-example translation slice, including live-package skeleton contracts.
