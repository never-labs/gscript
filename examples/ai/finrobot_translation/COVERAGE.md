...
- Current translation directory file count: 630 files, including 35
...
examples, and tests. Current inventory finds 13 registered generic live-package
examples covering model resolution, model IO envelopes, turns, tools, agents, workflows,
...
| Command | Result |
| --- | --- |
| `rg --files examples/ai/finrobot_translation \| wc -l \| tr -d ' '` | `630` files |
...
| `find examples/ai/finrobot_translation/live_packages -mindepth 1 -maxdepth 1 -type d ...` | `35` live-package skeleton directories |
| `find examples/ai/finrobot_translation/live_packages -path '*/fixtures/provider_free_fixture_index.json' -type f ...` | `32` provider-free fixture indexes |
| `go run ./cmd/leia examples list --json` filtered to `/live_packages/generic_` | `13` registered generic AI live-package examples |
...
This refresh updated the file-inventory count to 630 `rg --files` inventory files. The registered
...
| File inventory | 630 files in the translation directory, including checked-in live-package skeleton directories, status docs, and the upstream coverage ledger | Complete |
...
| Live-package skeletons | 35 checked-in skeleton directories, including 13 reusable `live_packages/generic_*` AI boundaries and 22 FinRobot/finance package boundaries; all 35 include registered `.leia` examples | Complete |
