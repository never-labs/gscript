...
- Current translation directory file count: 661 files, including 39
  provider-free live-package skeleton directories and the upstream coverage
  ledger artifact.
- Registered examples: 85 runnable/checkable examples discovered by
  `go run ./cmd/leia examples list --json`.
- Fixture index normalization: 38 live-package skeletons use
...
examples, and tests. Current inventory finds 17 registered generic live-package
examples covering model resolution, model IO envelopes, document RAG,
evidence/report artifacts, prompt/role catalogs, memory stores, turns, tools,
...
| Command | Result |
| --- | --- |
| `rg --files examples/ai/finrobot_translation \| wc -l \| tr -d ' '` | `661` files |
| `go run ./cmd/leia examples list --json` filtered to `examples/ai/finrobot_translation/` | `85` examples: `74` `host-vm`, `6` `llm-replay`, `5` `evaluate` |
| `find examples/ai/finrobot_translation/live_packages -mindepth 1 -maxdepth 1 -type d ...` | `39` live-package skeleton directories |
| `find examples/ai/finrobot_translation/live_packages -path '*/fixtures/provider_free_fixture_index.json' -type f ...` | `38` provider-free fixture indexes |
| `go run ./cmd/leia examples list --json` filtered to `/live_packages/generic_` | `17` registered generic AI live-package examples |
...
This refresh updated the file-inventory count to 661 `rg --files` inventory files. The registered
example, runner, live-package, generic live-package, and tutorial runnable
counts now include the generic evidence/report artifact package boundary.
...
Runner summary: 74 `host-vm`, 6 `llm-replay`, 5 `evaluate`, 85 runnable,
85 checkable.
...
| --- | --- | --- |
| Registered example inventory | 85 registered examples under `examples/ai/finrobot_translation`; all runnable/checkable | Complete |
| File inventory | 661 files in the translation directory, including checked-in live-package skeleton directories, status docs, and the upstream coverage ledger | Complete |
...
| Host-VM contract examples | 74 `host-vm` examples for config, tools, schemas, API replay, reports, compliance, packaging, and live-package skeleton contracts | Complete |
...
| Live-package skeletons | 39 checked-in skeleton directories, including 17 reusable `live_packages/generic_*` AI boundaries and 22 FinRobot/finance package boundaries; all 39 include registered `.leia` examples | Complete |
...
85-example translation slice, including live-package skeleton contracts.
