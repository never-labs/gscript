...
- Current translation directory file count: 867 files, including 53
...
- Fixture index normalization: 52 live-package skeletons use
...
examples, and tests. Current inventory finds 31 registered generic live-package
examples covering model resolution, model IO envelopes, coding workspaces,
data provider boundaries, document RAG, event intelligence boundaries, strategy simulation boundaries, evidence/report artifacts, report render
contracts, UI snapshot evaluation, chart render
contracts, optional adapter boundaries, product app boundaries, prompt/role catalogs, memory stores, turns, tools,
...
| `go run ./cmd/leia examples list --json` filtered to `/live_packages/generic_` | `31` registered generic AI live-package examples |
...
This refresh updated the file-inventory count to 867 `rg --files` inventory files. The registered
...
| `repo-ai-finrobot_translation-live_packages-generic_analytical_model_contracts-main` | `live_packages/generic_analytical_model_contracts/main.leia` | `host-vm` | Checked-in generic analytical model contracts skeleton for assumption audit, scenario books, sensitivity grids, method outputs, tolerance gates, provenance, and clean skips |
...
| `repo-ai-finrobot_translation-live_packages-generic_data_normalization_contracts-main` | `live_packages/generic_data_normalization_contracts/main.leia` | `host-vm` | Checked-in generic data normalization contracts skeleton for schema mappings, provider response projection, field policies, missing and stale handling, type coercion, unit transforms, provenance, validation envelopes, and clean skips |
| `repo-ai-finrobot_translation-live_packages-generic_data_provider_boundary-main` | `live_packages/generic_data_provider_boundary/main.leia` | `host-vm` | Checked-in generic data provider boundary skeleton for provider registries, request/response envelopes, pagination, rate limits, auth redaction, cache/retry policy, provenance, errors, and clean skips |
...
| `repo-ai-finrobot_translation-live_packages-generic_event_intelligence_boundary-main` | `live_packages/generic_event_intelligence_boundary/main.leia` | `host-vm` | Checked-in generic event intelligence boundary skeleton for source snapshots, event extraction, taxonomies, freshness, dedupe/source confidence, relevance, sentiment, impact, prompt contracts, and adapter clean skips |
...
| `repo-ai-finrobot_translation-live_packages-generic_evidence_verification-main` | `live_packages/generic_evidence_verification/main.leia` | `host-vm` | Checked-in generic evidence verification skeleton for document RAG evidence projection, claim records, source refs, citation normalization, freshness warnings, unresolved refs, quality summaries, and clean degradation |
| `repo-ai-finrobot_translation-live_packages-generic_evaluation_harness-main` | `live_packages/generic_evaluation_harness/main.leia` | `host-vm` | Checked-in generic AI evaluation harness skeleton for datasets, cases, metrics, replayed judges, findings, golden gates, case summaries, and agent-run projection |
| `repo-ai-finrobot_translation-live_packages-generic_strategy_backtest_contracts-main` | `live_packages/generic_strategy_backtest_contracts/main.leia` | `host-vm` | Checked-in generic strategy simulation contracts skeleton for strategy manifests, observation feeds, execution ledgers, allocation series, performance series, metric summaries, constraint limits, deterministic seeds, and engine clean skips |
...
| `repo-ai-finrobot_translation-live_packages-generic_transcript_pipeline-main` | `live_packages/generic_transcript_pipeline/main.leia` | `host-vm` | Checked-in generic transcript pipeline skeleton for source envelopes, speaker normalization, event-time correction, segment provenance, turn-aware chunking, and adapter clean skips |
| `repo-ai-finrobot_translation-live_packages-generic_turn_runner-main` | `live_packages/generic_turn_runner/main.leia` | `host-vm` | Checked-in generic AI turn runner skeleton for single-turn requests, memory context projection, response/usage/error envelopes, tool requests, and replay matching |
| `repo-ai-finrobot_translation-live_packages-generic_ui_snapshot_evaluator-main` | `live_packages/generic_ui_snapshot_evaluator/main.leia` | `host-vm` | Checked-in generic UI snapshot evaluator skeleton for evidence-report projection, route DOM schemas, viewport matrices, visual diff budgets, accessibility summaries, artifact URI manifests, redaction policy, static asset policy, and browser clean skips |
| `repo-ai-finrobot_translation-live_packages-generic_workflow_orchestrator-main` | `live_packages/generic_workflow_orchestrator/main.leia` | `host-vm` | Checked-in generic AI workflow orchestrator skeleton for workflow graphs, planning graph stage projection, stage I/O, handoff traces, retry/cache policy, workflow results, and trace hooks |
...
Runner summary: 88 `host-vm`, 6 `llm-replay`, 5 `evaluate`, 99 runnable,
99 checkable.
...
| File inventory | 867 files in the translation directory, including checked-in live-package skeleton directories, status docs, the structured gap manifest, and the upstream coverage ledger | Complete |
...
| Live-package skeletons | 53 checked-in skeleton directories, including 31 reusable `live_packages/generic_*` AI boundaries and 22 FinRobot/finance package boundaries; all 53 include registered `.leia` examples | Complete |
...
99-example translation slice, including live-package skeleton contracts.
