...
- Current translation directory file count: 800 files, including 51
  provider-free live-package skeleton directories and the upstream coverage
  ledger artifact.
- Registered examples: 97 runnable/checkable examples discovered by
  `go run ./cmd/leia examples list --json`.
- Fixture index normalization: 50 live-package skeletons use
...
examples, and tests. Current inventory finds 29 registered generic live-package
...
| Command | Result |
| --- | --- |
| `rg --files examples/ai/finrobot_translation \| wc -l \| tr -d ' '` | `800` files |
| `go run ./cmd/leia examples list --json` filtered to `examples/ai/finrobot_translation/` | `97` examples: `86` `host-vm`, `6` `llm-replay`, `5` `evaluate` |
| `find examples/ai/finrobot_translation/live_packages -mindepth 1 -maxdepth 1 -type d ...` | `51` live-package skeleton directories |
...
| `go run ./cmd/leia examples list --json` filtered to `/live_packages/generic_` | `29` registered generic AI live-package examples |
...
This refresh updated the file-inventory count to 800 `rg --files` inventory files. The registered
...
Runner summary: 86 `host-vm`, 6 `llm-replay`, 5 `evaluate`, 97 runnable,
97 checkable.
...
| --- | --- | --- |
| Registered example inventory | 97 registered examples under `examples/ai/finrobot_translation`; all runnable/checkable | Complete |
| File inventory | 800 files in the translation directory, including checked-in live-package skeleton directories, status docs, and the upstream coverage ledger | Complete |
...
| Host-VM contract examples | 86 `host-vm` examples for config, tools, schemas, API replay, reports, compliance, packaging, and live-package skeleton contracts | Complete |
...
| Live-package skeletons | 51 checked-in skeleton directories, including 29 reusable `live_packages/generic_*` AI boundaries and 22 FinRobot/finance package boundaries; all 51 include registered `.leia` examples | Complete |
...
97-example translation slice, including live-package skeleton contracts.
