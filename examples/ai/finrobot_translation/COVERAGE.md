...
- Current translation directory file count: 669 files, including 40
  provider-free live-package skeleton directories and the upstream coverage
  ledger artifact.
- Registered examples: 86 runnable/checkable examples discovered by
  `go run ./cmd/leia examples list --json`.
- Fixture index normalization: 39 live-package skeletons use
...
examples, and tests. Current inventory finds 18 registered generic live-package
examples covering model resolution, model IO envelopes, coding workspaces,
document RAG, evidence/report artifacts, prompt/role catalogs, memory stores, turns, tools,
...
| Command | Result |
| --- | --- |
| `rg --files examples/ai/finrobot_translation \| wc -l \| tr -d ' '` | `669` files |
| `go run ./cmd/leia examples list --json` filtered to `examples/ai/finrobot_translation/` | `86` examples: `75` `host-vm`, `6` `llm-replay`, `5` `evaluate` |
| `find examples/ai/finrobot_translation/live_packages -mindepth 1 -maxdepth 1 -type d ...` | `40` live-package skeleton directories |
| `find examples/ai/finrobot_translation/live_packages -path '*/fixtures/provider_free_fixture_index.json' -type f ...` | `39` provider-free fixture indexes |
| `go run ./cmd/leia examples list --json` filtered to `/live_packages/generic_` | `18` registered generic AI live-package examples |
...
This refresh updated the file-inventory count to 669 `rg --files` inventory files. The registered
example, runner, live-package, generic live-package, and tutorial runnable
counts now include the generic coding workspace package boundary.
...
Runner summary: 75 `host-vm`, 6 `llm-replay`, 5 `evaluate`, 86 runnable,
86 checkable.
...
| --- | --- | --- |
| Registered example inventory | 86 registered examples under `examples/ai/finrobot_translation`; all runnable/checkable | Complete |
| File inventory | 669 files in the translation directory, including checked-in live-package skeleton directories, status docs, and the upstream coverage ledger | Complete |
...
| Host-VM contract examples | 75 `host-vm` examples for config, tools, schemas, API replay, reports, compliance, packaging, and live-package skeleton contracts | Complete |
...
| Live-package skeletons | 40 checked-in skeleton directories, including 18 reusable `live_packages/generic_*` AI boundaries and 22 FinRobot/finance package boundaries; all 40 include registered `.leia` examples | Complete |
...
86-example translation slice, including live-package skeleton contracts.
