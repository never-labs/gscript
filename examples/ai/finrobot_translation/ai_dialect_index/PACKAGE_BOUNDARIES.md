...
- evidence-report-artifacts: `examples/ai/finrobot_translation/live_packages/generic_evidence_report_artifacts` (includes chart-render artifact projection)
...
| --- | --- | --- |
| Package composition | `generic_model_registry`, `generic_model_io_envelope`, `generic_memory_store`, `generic_turn_runner`, `generic_tool_contracts`, `generic_agent_runner`, `generic_planning_graph`, `generic_workflow_orchestrator`, `generic_product_app_boundary` | Leia scripts declare aliases, messages, tools, agents, memory, planning graphs, workflow graphs, and product shell boundaries; packages own validation, envelopes, context windows, memory-context projection into turn messages, tool-contract projection into agent tools, loop semantics, planning-to-stage projection, workflow-to-product projection, and stage I/O. |
...
| Agent state | `generic_agent_state_store`, `generic_memory_store`, `generic_turn_runner`, `generic_agent_runner`, `generic_trace_events`, `generic_record_replay` | Resumable snapshots, session identity, checkpoint/cache keys, redacted input and output refs, trace correlation, memory context projection, resume tokens, and clean skips are package-owned state contracts rather than hidden runtime state. |
