...
- `Mapped`: reviewed and mapped to a Leia target, with no runtime edit in this
  branch.
- `Partial`: a generic Leia surface exists conceptually, but needed contracts,
  examples, fixtures, or package APIs are incomplete.
- `Open`: requires package/runtime/product work before a faithful translation
  can be executable.
...
| `finrobot/data_source/yfinance_utils.py` | Yahoo Finance client package over `api`/`web` plus typed table outputs | Open | stdlib, external library | P0 | Offline fixtures for price history, company info, dividends, statements, cash flow, recommendations |
| `finrobot/data_source/finnhub_utils.py` | Finnhub client package with typed JSON decode, rate-limit metadata, and news/metrics schemas | Open | stdlib, external library | P0 | Recorded API fixtures validate profile, news, historical metrics, and basic financials |
| `finrobot/data_source/fmp_utils.py` | Financial Modeling Prep client package for targets, SEC metadata, market cap, BVPS, metrics, competitors | Open | stdlib, external library | P0 | Recorded fixtures cover pagination, errors, statement schemas, and competitor metrics |
| `finrobot/data_source/sec_utils.py` | SEC client/document package for 10-K metadata, downloads, PDF conversion, and section extraction | Open | stdlib, external library | P0 | Fixture-backed download and section-extraction tests over known 10-K filings |
| `finrobot/data_source/finnlp_utils.py`, `FinNLP/` | Optional FinNLP adapter package for news/social datasets | Open | stdlib, external library | P2 | Dataset download fixtures and schema normalization tests for selected providers |
| `finrobot/data_source/reddit_utils.py` | Reddit sentiment/source adapter with explicit capability and terms metadata | Open | stdlib, external library, product layer | P2 | Recorded fixture validates auth, pagination, post schema, and redaction |
| `finrobot/data_source/finance_data.py` | Generic finance data facade over provider adapters | Open | stdlib, external library | P1 | Contract test ensures facade returns typed tables with provenance |
| `finrobot/data_source/earnings_calls_src/*` | Earnings-call transcript package and RAG corpus builder | Open | stdlib, external library | P1 | Fixture transcript retrieval, chunking, metadata, and replayable RAG lookup |
| `finrobot/data_source/filings_src/*` | SEC filing search/fetch/section parser package | Open | stdlib, external library | P0 | Golden 10-K section fixtures, redirect handling, HTML/PDF parsing, and traceable errors |
| `finrobot/data_source/marker_sec_src/*` | PDF-to-markdown and SEC filing conversion tools | Open | stdlib, external library | P1 | Local PDF fixtures verify deterministic markdown chunks and artifact provenance |
...
| `finrobot/functional/charting.py` | Chart package examples for stock, share performance, P/E, EPS, and report charts | Open | stdlib, external library | P1 | Image/artifact tests validate chart metadata and deterministic fixture data |
...
| `finrobot/functional/quantitative.py` | Backtrader/strategy package wrapper | Open | stdlib, external library | P2 | Backtest fixture validates inputs, trades, metrics, and deterministic seed behavior |
| `finrobot/functional/reportlab.py` | Document/report package for annual-report PDF assembly | Open | stdlib, external library | P1 | Generated report fixture validates sections, assets, provenance, and disclosure markers |
...
| `experiments/portfolio_optimization.py` | Portfolio optimization package example | Open | stdlib, external library | P2 | Deterministic optimizer fixture validates constraints, weights, and risk metrics |
...
| `requirements*.txt`, `setup.py`, `Dockerfile`, `deploy*.sh`, `run_web_app.py` | Package/deploy metadata and local service entry points | Open | product layer, external library | P2 | Packaging smoke test and deployment doc check; no runtime coupling to core dialects |
...
| `finrobot_equity/core/src/create_equity_report.py`, `generate_pdf_report.py` | Report generation workflows over CSVs, charts, text sections, and PDF/HTML outputs | Open | stdlib, external library, product layer | P0 | Golden HTML/PDF artifact tests with stale-date and required-section checks |
| `finrobot_equity/core/src/modules/market_data_api.py` | FMP/YFinance market data package | Open | stdlib, external library | P0 | Recorded fixtures for statements, EV, ratios, target price, rating, profile, news, indicators |
| `finrobot_equity/core/src/modules/financial_data_processor.py` | Typed table normalization, historical metrics, and forecast transforms | Open | stdlib, external library | P0 | Unit tests for numeric cleaning, statement extraction, forecast formulas, and missing data |
| `finrobot_equity/core/src/modules/valuation_engine.py` | DCF, EV/EBITDA, peer comparison, synthesis, and football-field data package | Open | stdlib, external library | P1 | Deterministic valuation fixtures with sensitivity to assumptions |
| `finrobot_equity/core/src/modules/sensitivity_analyzer.py` | Sensitivity and confidence interval package | Open | stdlib, external library | P1 | Matrix/table fixtures for revenue and margin ranges |
...
| `finrobot_equity/core/src/modules/retail_sentiment_client.py` | Adanos/retail sentiment adapter over Reddit, X.com, and Polymarket snapshots | Open | stdlib, external library, product layer | P2 | Recorded snapshot fixtures validate source normalization and prompt formatting |
...
| `finrobot_equity/core/src/modules/chart_generator.py`, `enhanced_chart_generator.py` | Chart/report artifact package | Open | stdlib, external library | P1 | Fixture charts validate dimensions, source metadata, and report references |
...
| `finrobot_equity/core/src/modules/pdf_generator.py`, `professional_pdf_report.py` | PDF report package | Open | stdlib, external library, product layer | P1 | Rendered PDF fixture and page-level content checks |
...
See `GAPS.md` for the actionable gap backlog.
