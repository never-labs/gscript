...
Current status: `go run ./cmd/leia examples --json` discovers 28 runnable and
checkable FinRobot translation examples under this directory. The checked-in
live-package skeleton surface has four directories:
...
analytics report, optional integrations, and vendor adapters also provide
registered provider-free `.leia` examples, while product workflow is a
