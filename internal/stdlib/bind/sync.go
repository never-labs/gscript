...
import (
	...
	stdlibsync "github.com/never-labs/leia/internal/stdlib/lib/sync"
)
...
func syncGroupFromArgs(call ScriptFunctionCaller, launch TaskLauncher, args []Value) ([]Value, error) {
	if len(args) == 0 || args[0].IsNil() {
		...
		cancel := runtime.ScriptContextCancelValue(state)
...
func defaultSyncTaskLauncher(call ScriptFunctionCaller) TaskLauncher {
	return func(fn Value, args []Value, done func(error)) {
		go func() {
			...
			if call == nil {
				err = fmt.Errorf("sync.group: no script caller configured")
				return
			}
			_, err = call(fn, args)
		}()
...
func newScriptTaskGroupTable(call ScriptFunctionCaller, launch TaskLauncher, ctx, cancel Value) *Table {
...
func newScriptOnceTable(call ScriptFunctionCaller) *Table {
	...
	t.RawSetString("do", FunctionValue(&GoFunction{
		...
		Fn: func(args []Value) ([]Value, error) {
			...
			if call == nil {
				return nil, fmt.Errorf("sync.once.do: no script caller configured")
			}
			fn := args[0]
			state.once.Do(func() {
