...
import (
	"bufio"
	"fmt"
	hostio "github.com/never-labs/leia/internal/stdlib/lib/io"
...
func BuildIO(opts HostOptions) *Table {
	...
	set("lines", func(args []Value) ([]Value, error) {
		...
		if len(args) >= 1 && args[0].IsString() {
			var err error
			filename, err := resolveIOPath(root(), fsRead(), fsWrite(), args[0].Str(), true, false)
			if err != nil {
				return nil, err
			}
	...
	set("input", func(args []Value) ([]Value, error) {
		...
		h, err := inputOutputTarget(root(), fsRead(), fsWrite(), args[0], os.O_RDONLY)
		if err != nil {
	...
	set("output", func(args []Value) ([]Value, error) {
		...
		h, err := inputOutputTarget(root(), fsRead(), fsWrite(), args[0], os.O_WRONLY|os.O_CREATE|os.O_TRUNC)
		if err != nil {
			if strings.Contains(err.Error(), "filesystem ") {
				return nil, err
			}
...
func resolveIOPath(root string, filesystemRead, filesystemWrite bool, path string, read, write bool) (string, error) {
