...
func BuildProcessWithPolicy(opts HostOptions) *Table {
	t := NewTable()
	...
	set("run", func(args []Value) ([]Value, error) {
		...
		if len(args) > optsIndex && args[optsIndex].IsTable() {
			...
			if v := runOpts.RawGetString("dir"); v.IsString() {
				dir = v.Str()
				if HostString(opts.FilesystemRoot) != "" && opts.ResolveFilesystemPath != nil {
	...
	set("env", func(args []Value) ([]Value, error) {
		if !HostBool(opts.EnvironmentRead, true) {
	...
	set("entry", func(args []Value) ([]Value, error) {
		...
		tbl.RawSetString("dir", StringValue(HostString(opts.ScriptDir)))
