...
import (
	...
	utf8x "github.com/never-labs/leia/internal/stdlib/lib/utf8x"
)

// BuildUTF8 creates the "utf8" standard library table.
func BuildUTF8(maxHostResult func() int64) *Table {
	t := NewTable()
	...
	set("char", func(args []Value) ([]Value, error) {
		...
		for _, arg := range args {
			...
			if _, err := buf.Write([]byte(string(rune(n)))); err != nil {
				return nil, err
			}
		}
	...
	set("validate", func(args []Value) ([]Value, error) {
		...
		return []Value{TableValue(utf8ValidationReport(args[0].Str()))}, nil
	})
	...
	set("sanitize", func(args []Value) ([]Value, error) {
		...
		if err := CheckProjectedHostStringBytes(hostResultLimit(maxHostResult), len(out)); err != nil {
			return nil, err
		}
...
func utf8CodesIteratorValue(sv Value) Value {
	s := sv.Str()
	bytePos := -1
	iter := &GoFunction{Name: "utf8.codes_iterator"}
