...
import (
	...
	basemath "github.com/never-labs/leia/internal/stdlib/lib/math"
	baserand "github.com/never-labs/leia/internal/stdlib/lib/rand"
)

// BuildMath creates the "math" standard library table.
func BuildMath() *Table {
	t := NewTable()
	...
	set("atan", func(args []Value) ([]Value, error) {
		...
		y := mathToFloat(args[0])
		if len(args) >= 2 {
			x := mathToFloat(args[1])
	...
	if v := t.RawGetString("atan"); v.IsFunction() {
		gf := v.GoFunction()
		gf.FastArg1 = func(arg Value) (Value, error) {
			return FloatValue(math.Atan(mathToFloat(arg))), nil
		}
		gf.FastArg2 = func(a, b Value) (Value, error) {
			return FloatValue(math.Atan2(mathToFloat(a), mathToFloat(b))), nil
		}
		gf.Fast1 = func(args []Value) (Value, error) {
			if len(args) < 1 {
			...
			if len(args) >= 2 {
				return FloatValue(math.Atan2(mathToFloat(args[0]), mathToFloat(args[1]))), nil
			}
			return FloatValue(math.Atan(mathToFloat(args[0]))), nil
		}
	}
	...
	set("log", func(args []Value) ([]Value, error) {
		...
		x := mathToFloat(args[0])
		if len(args) >= 2 {
			base := mathToFloat(args[1])
	...
	if v := t.RawGetString("log"); v.IsFunction() {
		gf := v.GoFunction()
		gf.FastArg1 = func(arg Value) (Value, error) {
			return FloatValue(math.Log(mathToFloat(arg))), nil
		}
		gf.FastArg2 = func(a, b Value) (Value, error) {
			return FloatValue(math.Log(mathToFloat(a)) / math.Log(mathToFloat(b))), nil
		}
		gf.Fast1 = func(args []Value) (Value, error) {
			if len(args) < 1 {
			...
			x := mathToFloat(args[0])
			if len(args) >= 2 {
				return FloatValue(math.Log(x) / math.Log(mathToFloat(args[1]))), nil
			}
	...
	set("ult", func(args []Value) ([]Value, error) {
		...
		return []Value{BoolValue(uint64(mathToInt(args[0])) < uint64(mathToInt(args[1])))}, nil
	})
	if v := t.RawGetString("ult"); v.IsFunction() {
		gf := v.GoFunction()
		gf.FastArg2 = func(a, b Value) (Value, error) {
			return BoolValue(uint64(mathToInt(a)) < uint64(mathToInt(b))), nil
		}
		gf.Fast1 = func(args []Value) (Value, error) {
			if len(args) < 2 {
				return NilValue(), fmt.Errorf("bad argument to 'math.ult'")
			}
			return BoolValue(uint64(mathToInt(args[0])) < uint64(mathToInt(args[1]))), nil
		}
	}
	...
	set("pow", func(args []Value) ([]Value, error) {
		...
		return []Value{FloatValue(math.Pow(mathToFloat(args[0]), mathToFloat(args[1])))}, nil
	})
	if v := t.RawGetString("pow"); v.IsFunction() {
		gf := v.GoFunction()
		gf.FastArg2 = func(a, b Value) (Value, error) {
			return FloatValue(math.Pow(mathToFloat(a), mathToFloat(b))), nil
		}
		gf.Fast1 = func(args []Value) (Value, error) {
			if len(args) < 2 {
				return NilValue(), fmt.Errorf("bad argument to 'math.pow'")
			}
			return FloatValue(math.Pow(mathToFloat(args[0]), mathToFloat(args[1]))), nil
		}
	}
	...
	set("randomseed", func(args []Value) ([]Value, error) {
		...
		if len(args) == 0 {
			...
			randomSeed1 = mathToInt(args[0])
			if len(args) >= 2 {
				randomSeed2 = mathToInt(args[1])
			} else {
				randomSeed2 = 0
			}
		}
	...
	set("hypot", func(args []Value) ([]Value, error) {
		...
		return []Value{FloatValue(math.Hypot(mathToFloat(args[0]), mathToFloat(args[1])))}, nil
	})
	...
	set("isnan", func(args []Value) ([]Value, error) {
		...
		return []Value{BoolValue(math.IsNaN(mathToFloat(args[0])))}, nil
	})
	...
	set("isinf", func(args []Value) ([]Value, error) {
		...
		return []Value{BoolValue(math.IsInf(mathToFloat(args[0]), 0))}, nil
	})
...
func valueMathNumber(v Value) basemath.Number {
	...
	return basemath.Float(mathToFloat(v))
}
