...
func NewWithOptions(globals map[string]runtime.Value, opts Options) *VM {
	...
	v.RegisterStdlibRuntimeModules()
	v.RegisterStringLib()
	v.RegisterLLMLib()
	v.RegisterHTTPLib()
	v.RegisterDebugLib()
...
func (vm *VM) run() (retVals []runtime.Value, retErr error) {
	...
	for {
		...
		case OP_FRAME_FILTER:
			a := DecodeA(inst)
			b := DecodeB(inst)
			c := DecodeC(inst)
			frameVal := vm.regs[base+b]
			maskVal := vm.regs[base+c]
			if !maskVal.IsDenseArray() {
				return nil, wrapLineErr(frame, fmt.Errorf("FRAME_FILTER mask must be dense array (got %s)", maskVal.TypeName()))
			}
			...
			if !handled {
				return nil, wrapLineErr(frame, fmt.Errorf("FRAME_FILTER operand must be native frame (got %s)", frameVal.TypeName()))
			}
			vm.regs[base+a] = out
			if frame.closure.Proto.Feedback != nil {
				fb := &frame.closure.Proto.Feedback[frame.pc-1]
		...
		case OP_FRAME_GATHER:
			a := DecodeA(inst)
			b := DecodeB(inst)
			c := DecodeC(inst)
			frameVal := vm.regs[base+b]
			indexVal := vm.regs[base+c]
			if !indexVal.IsDenseArray() {
				return nil, wrapLineErr(frame, fmt.Errorf("FRAME_GATHER indexes must be dense array (got %s)", indexVal.TypeName()))
			}
			...
			if !handled {
				return nil, wrapLineErr(frame, fmt.Errorf("FRAME_GATHER operand must be native frame (got %s)", frameVal.TypeName()))
			}
			vm.regs[base+a] = out
			if frame.closure.Proto.Feedback != nil {
				fb := &frame.closure.Proto.Feedback[frame.pc-1]
		...
		case OP_FRAME_ORDER:
			...
			frameVal := vm.regs[base+b]
			if c >= len(constants) {
				return nil, wrapLineErr(frame, fmt.Errorf("FRAME_ORDER spec constant is out of range"))
			}
			...
			if !handled {
				return nil, wrapLineErr(frame, fmt.Errorf("FRAME_ORDER operand must be native frame (got %s)", frameVal.TypeName()))
			}
			vm.regs[base+a] = out
			if frame.closure.Proto.Feedback != nil {
				fb := &frame.closure.Proto.Feedback[frame.pc-1]
		...
		case OP_VECTOR_SCAN:
			...
			vectorVal := vm.regs[base+a]
			if !vectorVal.IsDenseArray() {
				return nil, wrapLineErr(frame, fmt.Errorf("VECTOR_SCAN operand must be dense array (got %s)", vectorVal.TypeName()))
			}
...
func frameOrderSpec(v runtime.Value) ([]string, []bool, int, error) {
	...
	if !v.IsTable() {
		return nil, nil, -1, fmt.Errorf("FRAME_ORDER spec must be a string or table")
	}
	tbl := v.Table()
	limit := -1
	if limitValue := tbl.RawGetString("limit"); !limitValue.IsNil() {
		if !limitValue.IsInt() {
			return nil, nil, -1, fmt.Errorf("FRAME_ORDER limit must be an integer")
		}
		if limitValue.Int() < 0 {
			return nil, nil, -1, fmt.Errorf("FRAME_ORDER limit must be non-negative")
		}
	...
	for i := 1; i <= n; i++ {
		...
		case item.IsTable():
			...
			if !col.IsString() {
				return nil, nil, -1, fmt.Errorf("FRAME_ORDER item %d must provide column", i)
			}
			names = append(names, col.Str())
			desc = append(desc, frameOrderTruthy(itemTable.RawGetString("desc")))
		default:
			return nil, nil, -1, fmt.Errorf("FRAME_ORDER item %d must be a string or table", i)
		}
	}
