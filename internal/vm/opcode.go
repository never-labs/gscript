...
const (
	...
	OP_VECTOR_WHERE_REDUCE         // A B C : R(A) = reduce(where(bool dense mask R(A), R(B), R(B+1)), dense-array reduce op C)
	OP_VECTOR_SCAN                 // A : R(A) = prefix-scan(dense-array R(A))
...
var opNames = [...]string{
	...
	OP_VECTOR_WHERE_REDUCE:         "VECTOR_WHERE_REDUCE",
	OP_VECTOR_SCAN:                 "VECTOR_SCAN",
