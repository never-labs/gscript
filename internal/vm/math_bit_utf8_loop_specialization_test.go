...
const mathBitUTF8LoopSpecializationSource = `
MOD := 1000000007
MASK32 := 4294967295
...
func TestMathBitUTF8LoopRuntimeSpecializationRecordsHit(t *testing.T) {
	stats := runtime.EnableRuntimePathStats()
	defer runtime.DisableRuntimePathStats()

	globals := compileAndRun(t, mathBitUTF8LoopSpecializationSource+`
result := bench(10)
`)
	if got := globals["result"]; !got.IsInt() || got.Int() != 516358877 {
		t.Fatalf("result = %s, want 516358877", got.String())
	}
	if got := runtimeRuntimeSpecializationHitCount(stats, RuntimeSpecializationRouteCallSiteValue, "math_bit_utf8_loop"); got != 1 {
		t.Fatalf("math_bit_utf8_loop hits = %d, want 1", got)
	}
}

func TestMathBitUTF8LoopRuntimeSpecializationGuardsStdlibMutation(t *testing.T) {
	stats := runtime.EnableRuntimePathStats()
	defer runtime.DisableRuntimePathStats()
...
result := bench(10)
`)
	if got := globals["result"]; !got.IsInt() {
		t.Fatalf("result = %s, want int", got.String())
	}
	if got := runtimeRuntimeSpecializationHitCount(stats, RuntimeSpecializationRouteCallSiteValue, "math_bit_utf8_loop"); got != 0 {
		t.Fatalf("math_bit_utf8_loop hits = %d, want 0", got)
	}
}
