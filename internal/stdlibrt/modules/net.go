...
func BuildNet(opts stdlibrt.HostOptions) *Table {
	t := markStdlibrtModule(NewTable())
	networkAllowed := func() bool { return stdlibrt.HostBool(opts.NetworkAllowed, true) }
	...
	set("get", func(args []Value) ([]Value, error) {
		...
		return netDoRequest(maxHostResult, "GET", url, "", opts)
	})
	...
	set("post", func(args []Value) ([]Value, error) {
		...
		return netDoRequest(maxHostResult, "POST", url, body, opts)
	})
	...
	set("put", func(args []Value) ([]Value, error) {
		...
		return netDoRequest(maxHostResult, "PUT", url, body, opts)
	})
	...
	set("delete", func(args []Value) ([]Value, error) {
		...
		return netDoRequest(maxHostResult, "DELETE", url, "", opts)
	})
	...
	set("patch", func(args []Value) ([]Value, error) {
		...
		return netDoRequest(maxHostResult, "PATCH", url, body, opts)
	})
	...
	set("request", func(args []Value) ([]Value, error) {
		...
		return netDoRequest(maxHostResult, method, url, body, args[0])
	})
