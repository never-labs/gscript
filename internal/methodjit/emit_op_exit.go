...
func (ec *emitContext) emitOpExit(instr *Instr) {
	...
	if (instr.Op == OpQVectorGatherReduce || instr.Op == OpFrameLen || instr.Op == OpFrameColumn || instr.Op == OpFrameMask || instr.Op == OpFrameProject || instr.Op == OpFrameFilter || instr.Op == OpFrameFilterProject || instr.Op == OpFrameGather || instr.Op == OpFrameSlice || instr.Op == OpFrameOrder || instr.Op == OpFrameOrderGather || instr.Op == OpFrameProjectColumn || instr.Op == OpFrameFilterProjectColumn || instr.Op == OpFrameGroupAggregate || instr.Op == OpQFrameSelectColumn) && tier2AltStackEnabled() && ec.selfCFCell != nil {
