...
func TestTableArrayBoundsCheckHoist_MarksHeaderBoundedLoad(t *testing.T) {
	...
	if !out.Analysis.LoopSpecializationFacts().TableArrayUpperBoundIsSafe(load.ID) {
		t.Fatalf("expected loop-header guard to prove TableArrayLoad upper bound:\n%s", Print(out))
	}
	if !out.Analysis.LoopSpecializationFacts().TableArrayLowerBoundIsSafe(load.ID) {
		t.Fatalf("expected non-negative induction to prove TableArrayLoad lower bound:\n%s", Print(out))
	}
	fact, ok := out.Analysis.LoopSpecializationFacts().LoopTableArrayFact(load.ID)
...
func TestLoopRegionVersioning_MarksCheckedStoreUpperBound(t *testing.T) {
	...
	if !out.Analysis.LoopSpecializationFacts().TableArrayUpperBoundIsSafe(load.ID) || !out.Analysis.LoopSpecializationFacts().TableArrayUpperBoundIsSafe(store.ID) {
		t.Fatalf("expected loop-region facts to prove load and store upper bounds:\n%s", Print(out))
	}
	if !out.Analysis.LoopSpecializationFacts().TableArrayLowerBoundIsSafe(load.ID) || !out.Analysis.LoopSpecializationFacts().TableArrayLowerBoundIsSafe(store.ID) {
		t.Fatalf("expected non-negative induction to prove load and store lower bounds:\n%s", Print(out))
	}
	storeFact, ok := out.Analysis.LoopSpecializationFacts().LoopTableArrayFact(store.ID)
...
func TestTableArrayStaticBounds_MarksSetListLoadWithRangedKey(t *testing.T) {
	...
	fn.Analysis.NumericFacts().SetIntRanges(map[int]intRange{key.ID: {min: 1, max: 3, known: true}})
...
func TestTableArrayStaticBounds_MarksDominatingLenGuardWithRangedKey(t *testing.T) {
	...
	fn.Analysis.NumericFacts().SetIntRanges(map[int]intRange{key.ID: {min: 1, max: 5, known: true}})
...
func TestTableArrayStaticBounds_UsesProfiledTableLenRange(t *testing.T) {
	...
	fn.Analysis.NumericFacts().SetProfiledLenRanges(map[int]intRange{tbl.ID: {min: 8, max: 8, known: true}})
...
func TestTableArrayStaticBounds_UsesDominatingTrueBranchKeyUpper(t *testing.T) {
	...
	fn.Analysis.NumericFacts().SetIntRanges(map[int]intRange{})
	fn.Analysis.NumericFacts().SetIntNonNegative(map[int]bool{key.ID: true})
...
func tableArrayBoundsCallLoopFixture(t *testing.T, passTargetTable bool) (*Function, *Instr) {
	...
	fn.Analysis.GlobalFacts().SetGlobals(map[string]*vm.FuncProto{
