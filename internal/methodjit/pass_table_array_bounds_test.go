...
func TestTableArrayBoundsCheckHoist_MarksHeaderBoundedLoad(t *testing.T) {
	...
	if out.Analysis.LoopSpecialization.TableArrayUpperBoundSafe == nil || !out.Analysis.LoopSpecialization.TableArrayUpperBoundSafe[load.ID] {
		t.Fatalf("expected loop-header guard to prove TableArrayLoad upper bound:\n%s", Print(out))
	}
	if out.Analysis.LoopSpecialization.TableArrayLowerBoundSafe == nil || !out.Analysis.LoopSpecialization.TableArrayLowerBoundSafe[load.ID] {
		t.Fatalf("expected non-negative induction to prove TableArrayLoad lower bound:\n%s", Print(out))
	}
	fact, ok := out.Analysis.LoopSpecialization.LoopTableArrayFacts[load.ID]
...
func TestLoopRegionVersioning_MarksCheckedStoreUpperBound(t *testing.T) {
	...
	if out.Analysis.LoopSpecialization.TableArrayUpperBoundSafe == nil || !out.Analysis.LoopSpecialization.TableArrayUpperBoundSafe[load.ID] || !out.Analysis.LoopSpecialization.TableArrayUpperBoundSafe[store.ID] {
		t.Fatalf("expected loop-region facts to prove load and store upper bounds:\n%s", Print(out))
	}
	if out.Analysis.LoopSpecialization.TableArrayLowerBoundSafe == nil || !out.Analysis.LoopSpecialization.TableArrayLowerBoundSafe[load.ID] || !out.Analysis.LoopSpecialization.TableArrayLowerBoundSafe[store.ID] {
		t.Fatalf("expected non-negative induction to prove load and store lower bounds:\n%s", Print(out))
	}
	storeFact, ok := out.Analysis.LoopSpecialization.LoopTableArrayFacts[store.ID]
...
func TestTableArrayStaticBounds_MarksSetListLoadWithRangedKey(t *testing.T) {
	...
	fn.Analysis.Numeric.IntRanges = map[int]intRange{key.ID: {min: 1, max: 3, known: true}}
...
func TestTableArrayStaticBounds_MarksDominatingLenGuardWithRangedKey(t *testing.T) {
	...
	fn.Analysis.Numeric.IntRanges = map[int]intRange{key.ID: {min: 1, max: 5, known: true}}
...
func TestTableArrayStaticBounds_UsesProfiledTableLenRange(t *testing.T) {
	...
	fn.Analysis.Numeric.ProfiledLenRanges = map[int]intRange{tbl.ID: {min: 8, max: 8, known: true}}
...
func TestTableArrayStaticBounds_UsesDominatingTrueBranchKeyUpper(t *testing.T) {
	...
	fn.Analysis.Numeric.IntRanges = map[int]intRange{}
	fn.Analysis.Numeric.IntNonNegative = map[int]bool{key.ID: true}
