...
func TestQFramePrimitiveLiteralMaskLowersToTypedRuntimeKernel(t *testing.T) {
	names := runtime.NewTable()
	names.RawSetInt(1, runtime.StringValue("price"))
	mask := runtime.NewTable()
	mask.RawSetString("column", runtime.StringValue("sym"))
	mask.RawSetString("op", runtime.StringValue("=="))
	mask.RawSetString("value", runtime.StringValue("AAPL"))
	mask.RawSetString("value_kind", runtime.StringValue("literal"))
	proto := &vm.FuncProto{
		Name:      "q_frame_literal_mask_pipeline_lowered",
		NumParams: 1,
		MaxStack:  2,
		Constants: []runtime.Value{
			runtime.TableValue(mask),
	...
	if err != nil {
		t.Fatalf("Interpret lowered literal-mask q hot path: %v", err)
	}
	if len(result) != 1 || !result[0].IsDenseArray() {
		t.Fatalf("lowered result = %#v, want one dense array", result)
	}
...
func TestQGroupAggregateStringWhereCallLowersToFilteredFrameGroupAggregateKernel(t *testing.T) {
	tests := []struct {
		...
		{name: "string", query: "select total:sum price, fills:count i by sym from trades where sym=\"AAPL\""},
		{name: "symbol", query: "select total:sum price, fills:count i by sym from trades where sym=`AAPL"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			proto := &vm.FuncProto{
				Name:      "q_group_aggregate_string_where_call_" + tt.name,
				NumParams: 1,
				MaxStack:  4,
				Constants: []runtime.Value{
					runtime.StringValue("q"),
...
func TestDiagnoseReportsStringFilteredQGroupAggregateRuntimeKernel(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			proto := &vm.FuncProto{
				Name:      "q_group_aggregate_string_filtered_diag_" + tt.name,
				NumParams: 1,
				MaxStack:  4,
				Constants: []runtime.Value{
					runtime.StringValue("q"),
...
func TestQFrameSelectColumnLiteralMaskUsesFusedHelper(t *testing.T) {
	names := runtime.NewTable()
	names.RawSetInt(1, runtime.StringValue("price"))
	mask := runtime.NewTable()
	mask.RawSetString("column", runtime.StringValue("sym"))
	...
	out, ok, err := executeQFrameSelectColumnCompareFilterProject(
		constants,
		spec,
		runtime.TableValue(qHotPathStringTestFrame(t)),
		runtime.NilValue(),
		false,
		[]string{"price"},
		"price",
	...
	if err != nil {
		t.Fatalf("execute q frame select-column literal mask: %v", err)
	}
	if !ok {
		t.Fatalf("execute q frame select-column literal mask returned ok=false, want fused helper path")
	}
...
func TestQEvalPipelinePlannerCopiesFusedDescriptorFields(t *testing.T) {
	...
	callableCount, ok := qRuntimeEvalPipelinePlanner{}.DescribeQEvalPipeline("f:{(+/x)+count y};.[f;(til 8;10#1)]")
	if !ok {
		t.Fatalf("DescribeQEvalPipeline did not recognize callable-dot count script pipeline")
	}
	if callableCount.Kind != "script" ||
		callableCount.Shape != "script-pipeline/callable-dot/sum-plus-count-right/assignments" ||
		callableCount.CallableExpr != "f" ||
		callableCount.ValueExpr != "til 8" ||
		callableCount.IndexExpr != "10#1" ||
