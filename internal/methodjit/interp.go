...
func (s *interpState) execInstr(instr *Instr, block *Block) ([]runtime.Value, bool, error) {
	...
	case OpFrameFilter:
		out, err := executeFrameFilterValue(s.val(instr.Args[0]), s.val(instr.Args[1]))
		if err != nil {
			return nil, false, err
		}
		s.values[instr.ID] = out
	...
	case OpFrameGroupAggregate:
		idx := int(instr.Aux)
		if idx < 0 || idx >= len(s.fn.Proto.Constants) {
			return nil, false, fmt.Errorf("FrameGroupAggregate spec constant is out of range")
		}
		out, err := executeFrameGroupAggregateValue(s.val(instr.Args[0]), s.val(instr.Args[1]), s.fn.Proto.Constants[idx])
		if err != nil {
			return nil, false, err
		}
		s.values[instr.ID] = out
	...
	case OpVectorWhere:
		out, err := executeVectorWhereValue(s.val(instr.Args[0]), s.val(instr.Args[1]), s.val(instr.Args[2]))
		if err != nil {
			return nil, false, err
		}
		s.values[instr.ID] = out
	...
	case OpVectorReduce:
		out, err := executeVectorReduceValue(int(instr.Aux), s.val(instr.Args[0]))
		if err != nil {
			return nil, false, err
		}
		s.values[instr.ID] = out
	...
	case OpQVectorWhereReduce:
		out, err := executeQVectorWhereReduceValue(int(instr.Aux), s.val(instr.Args[0]), s.val(instr.Args[1]), s.val(instr.Args[2]))
		if err != nil {
			return nil, false, err
		}
		s.values[instr.ID] = out
	...
	case OpQVectorGatherReduce:
		out, err := executeQVectorGatherReduceValue(int(instr.Aux), s.val(instr.Args[0]), s.val(instr.Args[1]))
		if err != nil {
			return nil, false, err
		}
		s.values[instr.ID] = out
