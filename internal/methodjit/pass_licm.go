...
func LICMPassCtx(ctx *PassContext) (*Function, error) {
	...
	return licmPass(ctx.Func(), globals, ctx.Call(), ctx.Numeric(), ctx.Speculation(), ctx.TableShape())
}

func licmPass(fn *Function, seededGlobals map[string]*vm.FuncProto, callFacts *CallFacts, numeric *NumericFacts, spec *SpeculationFacts, tableShapes *TableShapeFacts) (*Function, error) {
	...
	for _, h := range headers {
		...
		hoistOneLoop(fn, li, hdr, seededGlobals, callFacts, numeric, spec, tableShapes)
	}
...
func hoistOneLoop(fn *Function, li *loopInfo, hdr *Block, seededGlobals map[string]*vm.FuncProto, callFacts *CallFacts, numeric *NumericFacts, spec *SpeculationFacts, tableShapes *TableShapeFacts) {
	...
	for {
		...
		for _, loc := range inLoop {
			...
			if instr.Op == OpGetField {
				if len(instr.Args) >= 1 && licmLoopCallMayMutateValue(fn, loopCalls, instr.Args[0], seededGlobals, spec, tableShapes) {
					functionRemarks(fn).Add("LICM", "missed", loc.block.ID, instr.ID, instr.Op,
			...
			if instr.Op == OpGetGlobal || instr.Op == OpGuardGlobalConst {
				if licmLoopCallMayMutateGlobals(fn, loopCalls, seededGlobals, spec, tableShapes) {
					functionRemarks(fn).Add("LICM", "missed", loc.block.ID, instr.ID, instr.Op,
			...
			if instr.Op == OpGetUpval {
				if licmLoopCallMayMutateUpvalues(fn, loopCalls, seededGlobals, spec, tableShapes) {
					functionRemarks(fn).Add("LICM", "missed", loc.block.ID, instr.ID, instr.Op,
...
func licmLoopCallMayMutateGlobals(fn *Function, loopCalls []*Instr, seededGlobals map[string]*vm.FuncProto, spec *SpeculationFacts, tableShapes *TableShapeFacts) bool {
	for _, call := range loopCalls {
		if call == nil {
			continue
		}
		callees := licmCallCalleeProtos(fn, call, seededGlobals, spec, tableShapes)
		if len(callees) == 0 {
...
func licmLoopCallMayMutateUpvalues(fn *Function, loopCalls []*Instr, seededGlobals map[string]*vm.FuncProto, spec *SpeculationFacts, tableShapes *TableShapeFacts) bool {
	for _, call := range loopCalls {
		if call == nil {
			continue
		}
...
func licmLoopCallMayMutateValue(fn *Function, loopCalls []*Instr, value *Value, seededGlobals map[string]*vm.FuncProto, spec *SpeculationFacts, tableShapes *TableShapeFacts) bool {
	return licmLoopCallMayMutateValueWithGlobals(fn, loopCalls, value, seededGlobals, spec, tableShapes)
}

func licmLoopCallMayMutateValueWithGlobals(fn *Function, loopCalls []*Instr, value *Value, seededGlobals map[string]*vm.FuncProto, spec *SpeculationFacts, tableShapes *TableShapeFacts) bool {
	if value == nil {
		return true
	}
	for _, call := range loopCalls {
		...
		if !licmCallCannotMutateValue(fn, call, value.ID, seededGlobals, spec, tableShapes) {
			return true
		}
	}
	return false
...
func licmCallCannotMutateValue(fn *Function, instr *Instr, valueID int, seededGlobals map[string]*vm.FuncProto, spec *SpeculationFacts, tableShapes *TableShapeFacts) bool {
	if fn == nil || instr == nil {
		return false
	}
	callees := licmCallCalleeProtos(fn, instr, seededGlobals, spec, tableShapes)
	if len(callees) == 0 {
		return false
	}
	for _, callee := range callees {
...
func licmCallCalleeProtos(fn *Function, instr *Instr, seededGlobals map[string]*vm.FuncProto, spec *SpeculationFacts, tableShapes *TableShapeFacts) []*vm.FuncProto {
	if protos := fieldShapeCalleeProtosWithFacts(fn, tableShapes, instr); len(protos) > 0 {
		return protos
	}
	globals := callABIMergeGlobals(seededGlobals, callABIStableGlobals(fn.Proto))
