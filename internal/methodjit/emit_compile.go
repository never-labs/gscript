...
func fnHasResultProducingOpExit(fn *Function) bool {
	for _, block := range fn.Blocks {
		for _, instr := range block.Instrs {
			...
			switch instr.Op {
			case OpMatrixDense, OpFrameLen, OpFrameColumn, OpFrameMask, OpFrameProject, OpFrameFilter, OpFrameFilterProject, OpFrameGather, OpFrameSlice, OpFrameOrder, OpFrameProjectColumn, OpFrameFilterProjectColumn, OpVectorGather, OpVectorCompare, OpVectorMask, OpQFrameSelectColumn:
				return true
			}
		}
	}
