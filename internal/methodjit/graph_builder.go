...
func (b *graphBuilder) emitBlocks() {
	...
	for _, startPC := range blockStarts {
		...
		for pc := startPC; pc < endPC; pc++ {
			...
			case vm.OP_FRAME_MASK:
				a := vm.DecodeA(inst)
				bOp := vm.DecodeB(inst)
				c := vm.DecodeC(inst)
				frame := b.readVariable(bOp, block)
				instr := b.emit(block, OpFrameMask, TypeAny, []*Value{frame}, int64(c), 0)
				b.writeVariable(a, block, instr.Value())
			...
			case vm.OP_FRAME_FILTER_PROJECT:
				a := vm.DecodeA(inst)
				bOp := vm.DecodeB(inst)
				c := vm.DecodeC(inst)
				frame := b.readVariable(bOp, block)
				mask := b.readVariable(a, block)
				instr := b.emit(block, OpFrameFilterProject, TypeAny, []*Value{frame, mask}, int64(c), 0)
				b.writeVariable(a, block, instr.Value())

			case vm.OP_FRAME_GATHER:
				a := vm.DecodeA(inst)
				bOp := vm.DecodeB(inst)
				c := vm.DecodeC(inst)
				frame := b.readVariable(bOp, block)
				indexes := b.readVariable(c, block)
			...
			case vm.OP_FRAME_ORDER_GATHER:
				a := vm.DecodeA(inst)
				bOp := vm.DecodeB(inst)
				c := vm.DecodeC(inst)
				frame := b.readVariable(bOp, block)
				instr := b.emit(block, OpFrameOrderGather, TypeAny, []*Value{frame}, int64(c), 0)
				b.writeVariable(a, block, instr.Value())
			...
			case vm.OP_FRAME_GROUP_AGGREGATE:
				a := vm.DecodeA(inst)
				bOp := vm.DecodeB(inst)
				c := vm.DecodeC(inst)
				frame := b.readVariable(bOp, block)
				mask := b.readVariable(a, block)
				instr := b.emit(block, OpFrameGroupAggregate, TypeAny, []*Value{frame, mask}, int64(c), 0)
				b.writeVariable(a, block, instr.Value())
			...
			case vm.OP_VECTOR_WHERE:
				a := vm.DecodeA(inst)
				bOp := vm.DecodeB(inst)
				c := vm.DecodeC(inst)
				mask := b.readVariable(a, block)
				trueValue := b.readVariable(bOp, block)
				falseValue := b.readVariable(c, block)
				instr := b.emit(block, OpVectorWhere, TypeAny, []*Value{mask, trueValue, falseValue}, 0, 0)
				b.writeVariable(a, block, instr.Value())

			case vm.OP_VECTOR_REDUCE:
				a := vm.DecodeA(inst)
				c := vm.DecodeC(inst)
				vector := b.readVariable(a, block)
				instr := b.emit(block, OpVectorReduce, TypeAny, []*Value{vector}, int64(c), 0)
				b.writeVariable(a, block, instr.Value())

			case vm.OP_VECTOR_WHERE_REDUCE:
				a := vm.DecodeA(inst)
				bOp := vm.DecodeB(inst)
				c := vm.DecodeC(inst)
				mask := b.readVariable(a, block)
				trueValue := b.readVariable(bOp, block)
				falseValue := b.readVariable(bOp+1, block)
				instr := b.emit(block, OpQVectorWhereReduce, TypeAny, []*Value{mask, trueValue, falseValue}, int64(c), 0)
				b.writeVariable(a, block, instr.Value())

			case vm.OP_VECTOR_SCAN:
				a := vm.DecodeA(inst)
				vector := b.readVariable(a, block)
				instr := b.emit(block, OpVectorScan, TypeAny, []*Value{vector}, 0, 0)
				b.writeVariable(a, block, instr.Value())
