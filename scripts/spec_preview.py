...
GENERATED_SPEC_START = "<!-- BEGIN GENERATED SPEC CHAPTERS -->"
GENERATED_SPEC_END = "<!-- END GENERATED SPEC CHAPTERS -->"
...
def strip_generated_spec_chapters(text: str) -> str:
    start = text.find(GENERATED_SPEC_START)
    if start == -1:
        return text.rstrip() + "\n"
...
def main() -> int:
    ...
    parser.add_argument(
        "--write-index",
        action="store_true",
        help="regenerate docs/spec/index.md as the published single-page Markdown spec",
    )
    args = parser.parse_args()

    spec_dir = Path(args.spec_dir)
    if args.write_index:
        (spec_dir / "index.md").write_text(build_spec_index_markdown(spec_dir), encoding="utf-8")
