...
func TestScientificNumericExamplesSourceContract(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			summary: "ok kalman ",
			wantAPI: []string{"linalg.matrix", "linalg.row", "linalg.vector", "linalg.eye(2, 0.01)", "linalg.sandwich_add", "linalg.identity_minus", "linalg.rsolve", "linalg.T", "linalg.matmul", "linalg.trace", "linalg.at", "stats.rms", "math.near", "q {"},
		},
		{
			rel:     filepath.Join("examples", "scientific", "particle_filter.leia"),
			summary: "ok particle ",
			wantAPI: []string{"rand.seed", "stats.normal", "rand.sample", "linalg.add", "linalg.sub", "stats.uniform_weights", "stats.logpdf", "stats.importance_update", "stats.describe(particles, weights)", "math.near", "q {"},
		},
		{
