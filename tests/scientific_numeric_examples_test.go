...
func TestScientificNumericExamplesSourceContract(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			summary: "ok kalman ",
			wantAPI: []string{"linalg.matrix", "linalg.row", "linalg.vector", "linalg.eye(2, 0.01)", "linalg.sandwich_add", "linalg.sandwich", "linalg.matmul_t", "linalg.identity_minus", "linalg.rsolve", "linalg.matmul", "linalg.trace", "linalg.at", "stats.rms", "math.near", "q {"},
		},
		{
			rel:     filepath.Join("examples", "scientific", "particle_filter.leia"),
			summary: "ok particle ",
			wantAPI: []string{"rand.seed", "stats.normal", "rand.sample", "rand.add_noise", "stats.uniform_weights", "stats.loglik", "stats.bayes_update", "stats.describe(particles, weights)", "math.near", "q {"},
		},
		{
			rel:     filepath.Join("examples", "scientific", "inverted_pendulum.leia"),
			summary: "ok pendulum ",
			wantAPI: []string{"linalg.matrix", "linalg.diag", "control.lqr", "control.feedback", "ode.solve", "state_names", "wrap_angles", "final_state", "stats.describe_fields", "math.near", "q {"},
		},
	}
