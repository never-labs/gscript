...
- Suite: language conformance 5.4.8 tests
...
| `nextvar_table_insert_remove_proxy_metamethods` | `nextvar.lua`, `sort.lua` | `table.insert` and `table.remove` over proxy tables using `__len`, `__index`, and `__newindex`, including JIT conformance-check mode. |
