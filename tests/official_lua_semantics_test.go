...
func TestOfficialLuaTranslatedCases(t *testing.T) {
	root := findRepoRoot(t)
	...
	if luaBin == "" {
		...
		if err != nil {
			t.Skip("lua not found; set LUA_BIN to run official Lua translation checks")
		}
	}
	...
	if err != nil {
		t.Fatalf("glob official Lua cases: %v", err)
	}
	if len(luaCases) == 0 {
		t.Fatalf("no official Lua translated cases found in %s", caseDir)
	}
