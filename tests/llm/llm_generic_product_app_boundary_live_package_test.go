...
func TestGenericProductAppBoundaryLivePackageContractFixtureClosedLoop(t *testing.T) {
	...
	for _, want := range []string{"generic.ai.product_app.boundary", "generic.ai.product_app.route_contract", "generic.ai.product_app.session_contract", "generic.ai.product_app.task_log", "generic.ai.product_app.artifact_download", "generic.ai.product_app.crud_fixture_state", "generic.ai.product_app.db_migration_plan", "generic.ai.product_app.deployment_target", "generic.ai.product_app.approval_boundary", "generic.ai.product_app.clean_skip"} {
	...
	for _, want := range []string{"route_contracts", "session_contracts", "task_logs", "artifact_downloads", "crud_fixture_state", "db_migration_plan", "deployment_targets", "approval_boundaries", "clean_skips"} {
...
func TestGenericProductAppBoundaryLivePackageExecutableSkeleton(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			want := "generic_product_app_boundary_live_package capability=generic.ai.product_app.boundary entrypoint=ai.product_app.boundary routes=3 sessions=2 tasks=2 downloads=2 crud=3 migrations=1 deployments=2 approvals=2 clean_skip=3 provider_free=true live_network=false imports=false model_calls=false"
			if got != want {
