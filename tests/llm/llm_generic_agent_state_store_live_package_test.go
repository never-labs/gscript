...
func TestGenericAgentStateStoreLivePackageSchemaRequiredFields(t *testing.T) {
	...
	assertDocumentPipelineNestedSchemaRequired(t, filepath.Join(base, "schemas", "agent_state_snapshot_v1.schema.json"), []string{"properties", "state_snapshots", "items"}, []string{"agent_run_id", "session_id", "state_version", "resume_token", "status", "input_refs", "output_refs", "trace_correlation", "checkpoint", "redaction"})
	...
	assertDocumentPipelineNestedSchemaRequired(t, filepath.Join(base, "schemas", "redaction_audit_v1.schema.json"), []string{"properties", "credential_refs", "items"}, []string{"field", "value", "raw_value_stored"})
