...
| Local variable attributes | `constructs.lua`, `locals.lua`: `<const>`, `<close>` | 已补 Go-style `defer call(...)` / `defer obj:method(...)` 作为资源清理能力，注册时立即求值函数和参数，函数返回或出错时 LIFO 执行；不复刻 Lua `<close>`。只读局部 `const` 仍待按 GScript 语法设计。 |
