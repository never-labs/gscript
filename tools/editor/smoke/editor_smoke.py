...
def check_textmate() -> None:
    ...
    assert_match(leia, "meta.dialect.q.tagged-string.leia", "rows := q`select avg price by sym from trades`")
    assert_match(leia, "meta.dialect.q.tagged-string.leia", "rows := qsql`select avg price by sym from trades`")
    assert_any_match(leia, "entity.name.tag.dialect.q.leia", "rows := q`select avg price by sym from trades`")
    assert_any_match(leia, "keyword.control.qsql.q.leia", "rows := q`select avg price by sym from trades`")
    assert_any_match(leia, "support.function.q.leia", "rows := q`select avg price by sym from trades`")
    assert_any_match(leia, "constant.other.symbol.q.leia", "rows := q`flip `sym`price!(`AAPL;100.5)`")
