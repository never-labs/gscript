-- Benchmark: flat dense matmul with source-level 2-way sum splitting.
...
local function matmul(a, b, n)
    ...
    for i = 0, n - 1 do
        for j = 0, n - 1 do
            ...
            local k = 0
            while k + 1 < n do
                sum0 = sum0 + getf(a, i, k) * getf(b, k, j)
                sum1 = sum1 + getf(a, i, k + 1) * getf(b, k + 1, j)
                k = k + 2
            end
            local sum = sum0 + sum1
            while k < n do
...
print(string.format("matmul_dense_split2(%d) c[0][0] = %.6f", N, getf(c, 0, 0)))
