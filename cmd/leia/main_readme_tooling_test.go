...
func TestReadmeToolingCommandsMapToCLI(t *testing.T) {
	...
	for _, command := range commands {
		...
		case "ci":
			...
			for _, want := range []string{"bash scripts/performance_gate.sh --full", "bash scripts/production_check.sh --full --release-profile", "bash scripts/release_distribution_check.sh --require-goreleaser --require-workflows", "bash scripts/release_artifacts_check.sh --build"} {
				if !strings.Contains(stdout.String(), want) {
