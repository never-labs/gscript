...
`), 0644); err != nil {
	...
	if !graph.OK || graph.FileCount != len(graph.Files) || graph.DiagnosticCount != len(graph.Diagnostics) || len(graph.Files) != 1 || !containsString(graph.Files[0].Requires, "pkg.helper") || !containsString(graph.Files[0].Requires, "json") {
...
`), 0644); err != nil {
	...
	if !graph.OK || graph.FileCount != len(graph.Files) || graph.DiagnosticCount != len(graph.Diagnostics) || len(graph.Files) != 1 ||
