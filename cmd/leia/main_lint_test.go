...
func TestLintCommandUsesConfiguredFormat(t *testing.T) {
	dir := t.TempDir()
	if err := os.WriteFile(filepath.Join(dir, "leia.toml"), []byte("[tool.lint]\nformat = \"json\"\n"), 0644); err != nil {
		t.Fatal(err)
	}
	...
	if len(diagnostics) != 1 || diagnostics[0].File != path || diagnostics[0].Code != "LEIA1001" {
...
func TestLintLLMSyntaxCoverage(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "llm.leia")
...
func TestLintReportsSyntaxErrors(t *testing.T) {
	...
	out := stderr.String()
	for _, want := range []string{badPath, "LEIA1001", "parse error"} {
...
func TestLintJSONReportsSyntaxErrors(t *testing.T) {
	...
	if got.Code != "LEIA1001" {
		t.Fatalf("diagnostic code = %q, want LEIA1001", got.Code)
	}
...
func TestLintSARIFReportsSyntaxErrors(t *testing.T) {
	...
	if log.Runs[0].Tool.Driver.Name != "leia lint" {
		t.Fatalf("tool name = %q, want leia lint", log.Runs[0].Tool.Driver.Name)
	}
	if len(log.Runs[0].Results) != 1 {
		t.Fatalf("results len = %d, want 1", len(log.Runs[0].Results))
	}
	result := log.Runs[0].Results[0]
	if result.RuleID != "LEIA1001" || result.Level != "error" {
		t.Fatalf("result = %+v, want LEIA1001 error", result)
	}
