...
func TestCICommandListsProfiles(t *testing.T) {
	...
	out := stdout.String()
	for _, want := range []string{"go test", "./cmd/leia-lsp", "./internal/tooling/lsp", "scripts/run.sh manifest-check tests benchmarks", "scripts/run.sh module-path github.com/never-labs/leia", "leia check", "--no-docs", "--no-editor", "tests/smoke/01_basic.leia", "scripts/run.sh worktree"} {
