...
func TestCapabilitiesJSON(t *testing.T) {
	...
	for _, want := range []string{
		...
		"leia test --list --json",
		"leia version --json",
		"scripts/run.sh arch --json",
		"scripts/run.sh diagnostics --json",
		"scripts/run.sh docs --json",
		"scripts/run.sh editor --json",
		"scripts/install.sh --dry-run --json",
		"scripts/run.sh perf --validate-only FILE --json",
		"scripts/run.sh production --list --json",
		"scripts/run.sh public-blockers --json",
		"scripts/run.sh q --json",
		"scripts/run.sh release-artifacts --dry-run --json",
	...
	for _, tc := range []struct {
		...
		if report == nil || !report.Template {
			t.Fatalf("%s report capability = %+v, want template report", tc.command, report)
		}
		for _, param := range tc.params {
			if !capabilitiesHaveTemplateParameter(report.TemplateParameters, param) {
				t.Fatalf("%s template parameters = %+v, want %q", tc.command, report.TemplateParameters, param)
			}
		}
	}
	for _, tc := range []struct {
		...
		{"scripts/run.sh diagnostics --json", []string{"output_dir"}},
		{"scripts/run.sh editor --json", []string{"require_tree_sitter", "tree_sitter_status", "tree_sitter_command", "emacs_status", "emacs_command"}},
		...
		{"scripts/run.sh perf --validate-only FILE --json", []string{"validate_only", "timing_json", "validate_target.path", "validate_target.exists", "validate_target.is_file", "no_luajit", "threshold", "wall_threshold", "luajit_threshold"}},
		{"scripts/run.sh production --list --json", []string{"mode", "release_profile", "release_version", "output_dir", "list_only"}},
		{"scripts/run.sh public-blockers --json", []string{"require_resolved"}},
		{"scripts/run.sh q --json", []string{"scope", "bench_mode", "jobs", "timeout_seconds", "benchmark_json", "benchmark_markdown"}},
		{"scripts/run.sh release-artifacts --dry-run --json", []string{"dry_run", "output_dir", "version", "goos", "goarch", "git_commit", "git_branch", "git_dirty"}},
	...
	for _, tc := range []struct {
		...
		{"scripts/run.sh arch --json", "top_file_details"},
		{"scripts/run.sh arch --json", "large_file_details"},
		{"scripts/run.sh arch --json", "debt_marker_details"},
		{"scripts/run.sh arch --json", "missing_test_files"},
		{"scripts/run.sh diagnostics --json", "failure_details"},
		{"scripts/run.sh diagnostics --json", "files"},
		{"scripts/run.sh docs --json", "failures"},
		{"scripts/run.sh docs --json", "failure_kinds"},
		{"scripts/run.sh docs --json", "failure_details"},
		{"scripts/run.sh editor --json", "failure_kinds"},
		{"scripts/run.sh editor --json", "failure_details"},
		{"scripts/install.sh --dry-run --json", "install_entries"},
		{"scripts/run.sh production --list --json", "skipped_check_details"},
		{"scripts/run.sh production --list --json", "release_critical_runs"},
		{"scripts/run.sh production --list --json", "release_critical_skip_names"},
		{"scripts/run.sh production --list --json", "release_critical_skip_details"},
		{"scripts/run.sh public-blockers --json", "blocker_status_details"},
		{"scripts/run.sh q --json", "failure_kinds"},
		{"scripts/run.sh q --json", "failure_details"},
		{"scripts/run.sh release-artifacts --dry-run --json", "artifact_entries"},
		{"scripts/run.sh release-check --json", "artifact_entries"},
		{"scripts/run.sh release-check --json", "failure_kinds"},
		{"scripts/run.sh release-check --json", "failure_details"},
		{"scripts/run.sh release-dist --json", "failure_kinds"},
		{"scripts/run.sh release-dist --json", "failure_details"},
	...
	for _, tc := range []struct {
		...
		{"leia config --json", []string{"diagnostics[].severity", "diagnostics[].code", "diagnostics[].message", "diagnostics[].line"}},
		{"leia diag bundle --json", []string{"files[]"}},
		{"leia doc check --json", []string{"failures[]", "failure_kinds[]", "failure_details[].kind", "failure_details[].message"}},
		...
		{"leia inspect bytecode --json", []string{"proto.children[].name", "proto.children[].display_name", "proto.children[].instruction_count", "proto.children[].child_proto_count"}},
		{"leia inspect directives --json", []string{"directives[].kind", "directives[].text", "directives[].line", "directives[].column", "directives[].args"}},
		{"leia lint --json", []string{"diagnostics[].file", "diagnostics[].code", "diagnostics[].severity", "diagnostics[].message", "diagnostics[].line", "diagnostics[].column"}},
		{"leia mod capability --json", []string{"modules[].path", "modules[].kind", "modules[].root", "modules[].manifest", "modules[].capabilities"}},
		{"leia mod check --json", []string{"graph.files[].file"}},
		{"leia mod download --json", []string{"modules[].path", "modules[].version", "modules[].repo", "modules[].url", "modules[].zip", "modules[].extract_dir", "modules[].downloaded", "modules[].extracted"}},
		{"leia mod explain --json", []string{"diagnostics[].severity", "diagnostics[].code", "diagnostics[].message", "diagnostics[].file"}},
		{"leia mod graph --json", []string{"files[].file"}},
		{"leia mod list --json", []string{"requires[].path", "requires[].version", "requires[].kind", "requires[].source", "requires[].file", "replaces[].path", "replaces[].new_path", "replaces[].local", "replaces[].root", "collections[].name", "collections[].path", "collections[].root"}},
		{"leia mod lock --json", []string{"entries[].kind", "entries[].path", "entries[].target", "entries[].hash"}},
		{"leia mod gomod --json", []string{"diagnostics[].severity", "diagnostics[].code", "diagnostics[].message", "diagnostics[].file"}},
		{"leia mod tidy --json", []string{"removed[]"}},
		{"leia mod vendor --json", []string{"modules[].path", "modules[].version", "modules[].source", "modules[].target"}},
		{"leia mod verify --json", []string{"graph.files[].file"}},
		{"leia test --json", []string{"files[].file", "files[].ok"}},
		{"leia test --list --json", []string{"files[]"}},
		{"scripts/run.sh diagnostics --json", []string{"files[]"}},
		{"scripts/run.sh docs --json", []string{"failures[]", "failure_kinds[]", "failure_details[].kind", "failure_details[].message"}},
		{"scripts/run.sh editor --json", []string{"textmate_grammars[]", "vscode_assets[]", "tree_sitter_assets[]", "smoke_tests[]"}},
		{"scripts/install.sh --dry-run --json", []string{"install_entries[].role", "install_entries[].name", "install_entries[].path"}},
		{"scripts/run.sh perf --validate-only FILE --json", []string{"output_lines[]"}},
		{"scripts/run.sh production --list --json", []string{"runnable_checks[].name", "runnable_checks[].command", "runnable_checks[].release_critical"}},
		{"scripts/run.sh q --json", []string{"language_cases[]", "example_cases[]", "benchmark_cases[]"}},
		{"scripts/run.sh release-dist --json", []string{"install_target_details[].target", "install_target_details[].goos", "install_target_details[].goarch"}},
		{"scripts/run.sh release-notes --json", []string{"checked_file_details[].path", "checked_file_details[].role", "checked_file_details[].required", "checked_file_details[].exists"}},
	} {
		report := capabilitiesReport(caps.Tooling.Reports, tc.command)
	...
	for _, tc := range []struct {
		...
		{"scripts/run.sh arch --json", []string{"source_file_count", "source_line_count", "test_file_count", "test_line_count", "test_ratio_pct", "top_file_count", "large_file_count", "pass_pipeline_line_count", "debt_marker_count", "missing_test_count"}},
		...
		{"scripts/run.sh perf --validate-only FILE --json", []string{"validate_target.size_bytes", "failure_count", "failure_kind_count", "output_line_count"}},
		{"scripts/run.sh production --list --json", []string{"run_count", "skip_count", "release_critical_run_count", "critical_skip_count", "release_critical_skip_name_count"}},
		{"scripts/run.sh public-blockers --json", []string{"blocker_count", "missing_file_count", "release_decision_count", "stale_text_count", "unconfirmed_policy_count", "missing_guidance_count", "missing_doc_snippet_count", "open_blocker_count", "blocker_status_count", "decision_area_count"}},
		{"scripts/run.sh q --json", []string{"failure_kind_count", "failure_count", "language_case_count", "example_case_count", "benchmark_case_count"}},
		{"scripts/run.sh release-artifacts --dry-run --json", []string{"artifact_count", "checksum_entry_count"}},
...
func capabilitiesHaveTemplateParameter(params []cliReportTemplateParameter, name string) bool {
	for _, param := range params {
		if param.Name == name && strings.TrimSpace(param.Description) != "" {
			return true
		}
	}
	return false
