...
func TestCapabilitiesJSON(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"scripts/docs_check.sh --json", []string{"failures[]", "failure_kinds[]", "failure_details[].kind", "failure_details[].message"}},
		...
		{"scripts/performance_gate.sh --json", []string{"output_lines[]"}},
