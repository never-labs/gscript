...
func TestCapabilitiesJSON(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"scripts/run.sh perf --json", []string{"validate_only", "timing_json", "validate_target.path", "validate_target.exists", "validate_target.is_file", "no_luajit", "threshold", "wall_threshold", "luajit_threshold"}},
	...
	for _, tc := range []struct {
		...
		{"scripts/run.sh perf --json", []string{"output_lines[]"}},
	...
	for _, tc := range []struct {
		...
		{"scripts/run.sh perf --json", []string{"validate_target.size_bytes", "failure_count", "failure_kind_count", "output_line_count"}},
