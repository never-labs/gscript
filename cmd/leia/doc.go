...
func runDocCommand(args []string, outw, errw io.Writer) int {
	...
	case "site-check":
		return runDocSiteCheckCommand(args[1:], outw, errw)
	case "help", "-h", "--help":
		fmt.Fprintln(outw, "usage: leia doc [generate|check|spec-preview|site-check] [flags]")
		return 0
	default:
		fmt.Fprintf(errw, "leia doc: unknown mode %q (want generate, check, spec-preview, or site-check)\n", args[0])
		return 2
	}
}
...
func generateDialectReferenceMarkdown() []byte {
	...
	fmt.Fprintln(&b)
	fmt.Fprintln(&b, "Leia supports DSL-native tagged dialects for compact host automation, data format handling, web routing, spreadsheets, and optional LLM integrations. A dialect is an explicit tagged expression that returns an ordinary Leia value.")
	fmt.Fprintln(&b)
	...
	fmt.Fprintln(&b, "```")
	fmt.Fprintln(&b)
	fmt.Fprintln(&b, "The focused examples below show shell/process, data-format, spreadsheet, AI, and web dialect surfaces in small runnable programs.")
	fmt.Fprintln(&b)
