...
func generateDialectReferenceMarkdown() []byte {
	...
	fmt.Fprintln(&b, "```")
	fmt.Fprintln(&b)
	fmt.Fprintln(&b, "The release gate project combines fixture discovery, shell/process dialects, SQLite frames, q-style aggregation, spreadsheet round-tripping, a mocked AI agent, and a loopback web route. The focused examples below show the same dialect surface in smaller programs.")
	fmt.Fprintln(&b)
