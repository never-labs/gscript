...
func generateDialectReferenceMarkdown() []byte {
	...
	fmt.Fprintln(&b, "```")
	fmt.Fprintln(&b)
	fmt.Fprintln(&b, "The release gate project combines fixture discovery, shell/process dialects, SQLite frames, q-style aggregation, spreadsheet round-tripping, a mocked AI agent, and a loopback web route. It is tracked by the feature matrix so published dialect coverage stays tied to executable evidence.")
	fmt.Fprintln(&b)
	...
	fmt.Fprintln(&b, "- `examples/dialects/http_protocol_trace.leia`, `examples/dialects/sse_stream_fixture.leia`, and `examples/dialects/multipart_form_fixture.leia` for web/protocol tags.")
