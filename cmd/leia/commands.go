...
func cliCommands() []cliCommandSpec {
	return []cliCommandSpec{
		{Name: "bench", Usage: "usage: leia bench [--manifest-check|--quick|--full|--guard|BENCH|compare [--quick]|strict|diagnose|triage|audit|rank-luajit-gaps|debug-artifact|coverage|profile-exits|validate-lua-refs|submit-guard|jit-addr-map|regression-guard] [benchmark-harness-flags...]", Summary: "Run benchmark and benchmark-diagnostic harnesses.", Run: runBenchCommand},
		...
		{Name: "doc", Usage: "usage: leia doc generate [flags]; leia doc check [--json]; leia doc site-check [--site-dir DIR] [--base-path PATH] [--json]", Summary: "Generate reference docs or validate repository docs and rendered site output.", Run: runDocCommand},
