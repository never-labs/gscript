...
func buildReportCapabilities() []cliReportCapability {
	return []cliReportCapability{
		...
		{Command: "scripts/public_release_blockers_check.sh --json", Formats: []string{"json"}, SchemaVersion: 1, StatusField: "status", ScalarFields: []string{"require_resolved"}, CountFields: []string{"blocker_count", "missing_file_count", "release_decision_count", "stale_text_count", "unconfirmed_policy_count", "missing_guidance_count", "missing_doc_snippet_count", "decision_area_count"}, CollectionFields: []string{"blockers", "blocker_details", "decision_areas"}},
