...
func buildReportCapabilities() []cliReportCapability {
	return []cliReportCapability{
		...
		{Command: "leia ci --list --json", Formats: []string{"json"}, SchemaVersion: 1, StatusField: "status", CountFields: []string{"command_count", "commands[].arg_count"}, CollectionFields: []string{"commands", "commands[].args"}},
		...
		{Command: "scripts/diagnostics_bundle.sh --json", Formats: []string{"json"}, SchemaVersion: 1, StatusField: "status", ScalarFields: []string{"output_dir"}, CountFields: []string{"failure_count", "file_count"}, CollectionFields: []string{"files"}},
		{Command: "scripts/docs_check.sh --json", Formats: []string{"json"}, SchemaVersion: 1, StatusField: "status", CountFields: []string{"failure_count", "counts.markdown_files", "counts.relative_documentation_links", "counts.runnable_spec_examples"}, CollectionFields: []string{"failures"}},
		{Command: "scripts/editor_check.sh --json", Formats: []string{"json"}, SchemaVersion: 1, StatusField: "status", ScalarFields: []string{"require_tree_sitter", "tree_sitter_status", "tree_sitter_command", "emacs_status", "emacs_command"}, CountFields: []string{"textmate_grammar_count", "vscode_asset_count", "tree_sitter_asset_count", "smoke_test_count"}, CollectionFields: []string{"textmate_grammars", "vscode_assets", "tree_sitter_assets", "smoke_tests"}},
		{Command: "scripts/install.sh --dry-run --json", Formats: []string{"json"}, SchemaVersion: 1, StatusField: "status", ScalarFields: []string{"dry_run", "verify", "repo", "version", "goos", "goarch", "archive_ext", "asset", "url", "checksums", "bin_dir", "binary", "lsp_binary", "install_path", "lsp_install_path"}, CountFields: []string{"install_count", "binary_count", "install_path_count"}, CollectionFields: []string{"binaries", "install_paths"}},
		{Command: "scripts/performance_gate.sh --json", Formats: []string{"json"}, SchemaVersion: 1, StatusField: "status", ScalarFields: []string{"validate_only", "timing_json", "no_luajit", "threshold", "wall_threshold", "luajit_threshold"}, CountFields: []string{"failure_count", "output_line_count"}, CollectionFields: []string{"failures", "output_lines"}},
		{Command: "scripts/production_check.sh --list --json", Formats: []string{"json"}, SchemaVersion: 1, StatusField: "status", ScalarFields: []string{"mode", "release_profile", "release_version", "output_dir", "list_only"}, CountFields: []string{"run_count", "skip_count", "critical_skip_count", "release_critical_skip_name_count"}, CollectionFields: []string{"runnable_checks", "skipped_checks", "release_critical_skip_names", "release_critical_skips"}},
		{Command: "scripts/public_release_blockers_check.sh --json", Formats: []string{"json"}, SchemaVersion: 1, StatusField: "status", ScalarFields: []string{"require_resolved"}, CountFields: []string{"blocker_count", "missing_file_count", "release_decision_count", "stale_text_count", "unconfirmed_policy_count", "missing_guidance_count", "missing_doc_snippet_count", "open_blocker_count", "blocker_status_count", "decision_area_count"}, CollectionFields: []string{"blockers", "blocker_details", "blocker_statuses", "decision_areas"}},
		{Command: "scripts/q_conformance_gate.sh --json", Formats: []string{"json"}, SchemaVersion: 1, StatusField: "status", ScalarFields: []string{"scope", "bench_mode", "jobs", "timeout_seconds", "benchmark_json", "benchmark_markdown"}, CountFields: []string{"language_case_count", "example_case_count", "benchmark_case_count"}, CollectionFields: []string{"language_cases", "example_cases", "benchmark_cases"}},
		{Command: "scripts/release_artifacts.sh --dry-run --json", Formats: []string{"json"}, SchemaVersion: 1, StatusField: "status", ScalarFields: []string{"dry_run", "output_dir", "version", "goos", "goarch", "git_commit", "git_branch", "git_dirty"}, CountFields: []string{"artifact_count", "checksum_entry_count"}, CollectionFields: []string{"artifact_files"}},
		{Command: "scripts/release_artifacts_check.sh --json", Formats: []string{"json"}, SchemaVersion: 1, StatusField: "status", ScalarFields: []string{"version", "build", "require_clean", "require_tag", "goos", "goarch", "dry_run_verified", "build_verified", "install_archive_verified", "output_dir"}, CountFields: []string{"artifact_count", "checksum_entry_count", "install_archive_checksum_count"}, CollectionFields: []string{"artifact_files"}},
		{Command: "scripts/release_distribution_check.sh --json", Formats: []string{"json"}, SchemaVersion: 1, StatusField: "status", ScalarFields: []string{"require_goreleaser", "require_workflows", "goreleaser_available", "local_install_fixture"}, CountFields: []string{"workflow_count", "install_target_count"}, CollectionFields: []string{"workflow_files", "install_targets"}},
		{Command: "scripts/release_notes_check.sh --json", Formats: []string{"json"}, SchemaVersion: 1, StatusField: "status", ScalarFields: []string{"require_ready", "version"}, CountFields: []string{"checked_file_count", "required_artifact_count", "artifact_checksum_count", "failure_kind_count", "failure_count"}, CollectionFields: []string{"checked_files", "failure_kinds", "failures", "failure_details"}},
		{Command: "scripts/worktree_audit.sh --json", Formats: []string{"json"}, SchemaVersion: 1, StatusField: "status", ScalarFields: []string{"fail_on_findings"}, CountFields: []string{"finding_count"}, CollectionFields: []string{"findings"}},
	}
}
