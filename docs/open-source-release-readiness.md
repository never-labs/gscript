...
This draft scopes the non-language work required before presenting GScript as a
...
- [GScript CLI and Toolchain Audit](cli-audit.md) defines the standalone
...
- a clear product position: embeddable Go scripting runtime plus standalone
  GScript CLI, with AI-native syntax as the differentiating demo surface;
...
|---|---|---|---|---|
| Positioning and README | `README.md` has a short identity sentence and basic commands. | It does not explain who should use GScript, what is stable, how AI-native work differs from ordinary embedding, or what is explicitly experimental. | Rewrite the README into a product entry point: value proposition, 60-second install/run path, VM/JIT support matrix, AI-native demo link, embedding demo link, docs map, maturity statement, and support policy. | A new user can run one script and one embedding example from README commands without reading repository internals. |
...
| Package management and ecosystem | `docs/tooling.md` describes early `gscript mod` and no networked registry. | Language users lack manifest/lockfile docs, module naming rules, version constraints, and package discovery. | For `v0.1.0`, explicitly scope package management as local-only. Document `gscript.toml`, `require()` resolution, vendoring/non-goals, and future registry design. | README and docs do not imply a package registry exists; local module examples work from a clean checkout. |
| Community templates | No `.github` directory is visible. | Issues and PRs will arrive without repro data, platform info, security routing, or benchmark artifacts. | Add issue templates for bug, performance regression, language proposal, docs issue, security redirect, and feature request. Add PR template with required test/docs/perf evidence. | New issues collect OS/arch, Go version, GScript version, command, expected/actual output, and minimal repro. |
| AI-native killer demo | `docs/ai-native-syntax-design.md` and `docs/agent-spec.md` define the direction. | The public release needs one real demo that justifies a new language rather than a Go library wrapper. | Build a flagship demo: a constrained AI agent that reads project docs, runs `gscript check`/tests, uses tools, respects budget/security policy, and produces a patch or release report. Include mock-provider mode and live-provider mode. | Demo runs offline with mock responses in CI and live with documented `OPENAI_API_KEY` or compatible provider env vars. |
...
```bash
go run ./cmd/gscript examples/ai_native/release_assistant_mock.gs
go run ./cmd/gscript check examples/ai_native/release_assistant_mock.gs
...
| Write GScript | language spec, stdlib, AI-native syntax, tooling |
| Embed GScript | Go API, host functions, sandboxing, cancellation, stdio hooks |
| Operate GScript | security, diagnostics, release, compatibility, benchmark methodology |
...
- a project manifest such as `gscript.toml` if the command already supports it;
...
The flagship demo should prove something specific: GScript can express an agent
workflow with less host glue while still preserving tool boundaries, budgets,
and sandbox policy.

Recommended demo: `examples/ai_native/release_assistant.gs`.

Behavior:

1. Reads the project README and selected docs.
2. Runs `gscript version --json`, `gscript check`, and optionally
...
```bash
go run ./cmd/gscript examples/ai_native/release_assistant_mock.gs
go run ./cmd/gscript examples/ai_native/release_assistant_mock.gs --json
```
