...
|---|---|---|
| `bench` | `usage: leia bench [--manifest-check|--quick|--full|--guard|BENCH|compare [--quick]|strict|diagnose|triage|q-report|audit|rank-luajit-gaps|debug-artifact|coverage|profile-exits|validate-lua-refs|submit-guard|jit-addr-map|regression-guard] [benchmark-harness-flags...]` | Run benchmark and benchmark-diagnostic harnesses. |
