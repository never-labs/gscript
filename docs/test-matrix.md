...
| `gscript/gscript_test.go` sandbox/capability cases + `docs/security.md` / `docs/stdlib.md` / `docs/embedding.md` | `LibFlags` vs `CapabilityFlags`、`WithSandbox` 默认边界、`CapFilesystemRead`/`CapFilesystemWrite`、`CapFilesystem` 兼容别名、filesystem root confinement、module loading policy | `go test ./gscript -run 'TestWith(Sandbox|ModuleLoading|FilesystemRoot|FilesystemFalse|FilesystemRead|FilesystemWrite|Libs)' -count=1`；release metadata 用 `go test ./tests -run 'TestFeatureMatrixSchema|TestReleaseMatrix' -count=1` | 验证 `LibSafe`/`CapSafe` 分层、stdlib require 与文件模块 require 的边界、`fs`/`dofile`/`loadfile` 禁用、读写 filesystem capability 拆分、root 外路径拒绝；`tests/feature_matrix.json` 的 `sandbox_capabilities_module_loading` 条目把这些文档和测试作为 release coverage refs。 |
...
| P0 sandbox/capability docs | `go test ./gscript -run 'TestWith(Sandbox|ModuleLoading|FilesystemRoot|FilesystemFalse|FilesystemRead|FilesystemWrite|Libs)' -count=1` + `go test ./tests -run 'TestFeatureMatrixSchema|TestReleaseMatrix' -count=1` | sandbox、stdlib host 能力、embedding API、module loading 文档或行为改动 | `LibFlags`/`CapabilityFlags` 分层文档缺 release refs；`WithSandbox` 不再等于 `LibSafe` + `CapSafe`；`CapFilesystemRead`/`CapFilesystemWrite`、`CapFilesystem` 兼容别名、stdlib require、文件模块 require、filesystem root confinement 行为与文档不一致。 |
