...
| `tests/*.gs` + `tests/*_test.go` | 手写语言 smoke、feature matrix、CLI/JIT 行为 | `go test ./tests -count=1 -p 1 -timeout=600s` | 覆盖 parser/runtime/VM/JIT 的端到端语言行为；`tests/feature_matrix.json` 记录 feature 到 semantic/perf 覆盖的映射。 |
...
| P0 coverage metadata | `go test ./tests -run TestFeatureMatrixSchema -count=1` | 每个 PR/提交 | `tests/feature_matrix.json` schema、status、repo-relative ref 失效。 |
...
3. 每次提交显式执行 `go test ./tests -run TestOfficialLuaTranslatedCases -count=1 -v`，并把 skip 视为配置失败。
4. JIT/runtime/parser/stdlib 改动追加 `GSCRIPT_OFFICIAL_CHECK_JIT=1 go test ./tests -run TestOfficialLuaTranslatedCases -count=1 -v`。
