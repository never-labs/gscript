# Leia CLI and Toolchain Audit

本文审计当前 CLI/工具链形态，并给出把 Leia 作为独立语言发布前需要收敛的命令集合与落地顺序。范围限于用户可见命令、开发者工具入口、benchmark/test 入口，以及这些入口和 Go embedding API 的边界。

## Current Command Shape

### `cmd/leia`

`cmd/leia/main.go` 是当前主入口，采用 Go `flag` 包的一层平铺 flags：

- `leia -e SOURCE [args...]` 执行字符串。
- `leia` 无参数进入最小 REPL。
- `leia FILE [args...]` 执行文件。
...
`cmd/dump_bytecode` 接受 `FILE`，递归打印 `<main>` 和所有 child proto 的 bytecode，并附带 params、maxstack、vararg、JIT disabled、Tier 1/Tier 2 callable decision。它是有价值的 inspect 能力，但当前作为独立开发者二进制存在，不在 `leia` 用户命令树里。
...
- `tests/integration_test.go` 直接通过 lexer/parser/interpreter 执行 `tests/01_basic.leia` 到 `tests/12_advanced.leia`。
- `tests/official_lua_semantics_test.go` 构建 `./cmd/leia`，用 `lua` 或 `LUA_BIN` 作为 oracle，对 `tests/language/*.lua` 与对应 `.leia` 比较输出；设置 `LEIA_OFFICIAL_CHECK_JIT=1` 时额外比较 `leia -jit`。
...
用户视角还没有 `leia test`。测试能力存在，但它绑定 Go module、Go test package、环境变量和 repo layout，不是语言用户能直接使用的项目级 test runner。
...
- benchmark harness 很强，但名字、参数和输出属于仓库维护工具，不是 `leia bench` 的产品化接口。
- 测试入口依赖 `go test` 和 translated official oracle；没有 Leia 原生测试文件约定、断言库、fixture、snapshot、coverage 或 watch mode。
...
第一阶段应保留现有 `leia FILE`、`leia -e`、`leia` 兼容入口，但新增子命令，并把新文档只承诺子命令面。

| Command | Purpose |
|---|---|
| `leia run FILE [-- args...]` | 执行脚本；承载 `--vm`、`--jit`、profile、runtime args、diagnostics。 |
| `leia eval SOURCE [-- args...]` | 执行字符串，替代直接暴露 `-e` 作为主体验。 |
| `leia repl` | 交互式 shell，显示 engine、history、completion、multiline 输入。 |
| `leia check PATH...` | parse/compile/type-adjacent 静态检查，不运行程序。 |
| `leia fmt PATH...` | parser-backed formatter，支持 `--check`、`--write`、stdin。 |
| `leia lint PATH...` | 静态诊断，输出 text/json/sarif，使用稳定 diagnostic codes。 |
| `leia test [PATH...]` | 原生测试 runner，支持 package/project discovery、filter、timeout、JSON。 |
| `leia bench [PATH...]` | 用户级 benchmark runner；内部可调用现有 harness，但输出稳定。 |
| `leia diag ...` | JIT/runtime 诊断入口，收敛现有 `-exit-stats-json`、runtime path、warm dump、spec worklist。 |
| `leia inspect bytecode FILE` | 收敛 `cmd/dump`、`cmd/dump_bytecode`，支持 `--proto`、`--json`。 |
| `leia doc [PATH...]` | 生成 API/stdlib/project docs。 |
| `leia mod init/tidy/graph/vendor` | 项目和依赖管理。 |
| `leia pkg publish/install/update/audit` | registry/package 操作；可在 registry 就绪前先保留 experimental。 |
| `leia env` | 打印 toolchain、cache、config、platform、JIT availability。 |
| `leia version --json` | 稳定版本和 build metadata。 |
| `leia capabilities --json` | 机器可读能力：engine、JIT、stdlib、diagnostic、platform、benchmark deps。 |
...
- `leia run/check/test/bench` 可以调用同一组 public/internal service packages，但 public Go API 应提供结构化 options/result，而 CLI 只是其中一个 adapter。
- `cmd/dump`/`dump_bytecode` 的能力应先产品化为 `inspect` 输出；embedding API 只在确有外部需求时提供稳定 introspection object，避免过早承诺 bytecode layout。
- Benchmark harness 可以继续作为维护者工具存在；`leia bench` 应定义更小、更稳定的用户契约，再选择性复用这些 harness。

## Landing Plan

### P0

- 新增 `leia run/eval/repl` 子命令，同时保留旧入口兼容；文档把子命令定义为推荐入口。
- 定义全局 exit-code contract 和 JSON envelope v1。
- 把 `cmd/dump`、`cmd/dump_bytecode` 收敛为 `leia inspect bytecode FILE [--proto NAME] [--json]`。
- 新增 `leia check`：只做 lex/parse/compile，不运行程序，输出稳定 source diagnostics。
- 新增 `leia capabilities --json` 与 `leia version --json`，让 CI/IDE 能探测 JIT、stdlib 和平台能力。
- 为现有 benchmark/test 文档补一张“维护者入口 vs 未来用户入口”映射，避免继续扩散零散命令。

### P1

- 新增 `leia test`，先支持 repo 内 `.leia` test convention、filter、timeout、JSON；official Lua oracle 保持维护者 gate，不作为普通用户默认依赖。
- 新增 `leia fmt --check/--write/stdin`，基于 parser/AST/golden tests，不做 regex formatter。
- 新增 `leia lint --format=text|json|sarif`，定义 diagnostic code registry、severity、suppression。
- 新增 `leia bench` 的稳定用户层，内部可复用 `timing_compare.py`/`strict_guard.py`，但输出使用 CLI envelope。
- 把现有 JIT flags 分组到 `leia diag`，保留 run flags 的兼容 alias。
- 建立项目配置文件和项目根发现规则，例如 `leia.toml`。

### P2

- 新增 `leia mod` 和 `leia pkg`，覆盖 manifest、lockfile、dependency graph、registry、publish/install/update/audit。
- 新增 `leia doc` 和 stdlib/project documentation generation。
...
不要继续新增独立 `cmd/*` 或一次性 shell wrapper 作为用户入口。短期最佳路径是先建立 `leia` 子命令框架和输出契约，把已有强能力包进稳定命令面；随后再补 fmt/lint/test/bench/package 等独立语言发布必需的用户级工具。
