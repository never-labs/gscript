...
- `WithMaxHostResultBytes` limits string bytes returned from one native Go
  ...
  CSV encoding, and compressed decode output are bounded while they are
  captured or expanded.
...
- Filesystem policy currently has read/write capability bits plus one root
  ...
  audit events.
- No public host policy for broader process control or host callbacks beyond
  coarse stdlib removal.
